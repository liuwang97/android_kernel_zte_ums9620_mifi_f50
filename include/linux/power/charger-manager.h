...
struct charger_desc {
	...
	const char *zte_psy_fuel_gauge;

	const char *thermal_zone;
	const char *psy_hardware;
	const char *psy_hardware2;

	const char *zte_psy_hardware;
	const char *zte_psy_hardware2;
	...
	struct charger_manager_charge_current *cm_charge_current;
	u32 cp_nums;
	u32 psy_cp_nums;
...
struct charger_manager_charge_current {
	int sdp_limit;
	int sdp_cur;
	int dcp_limit;
	int dcp_cur;
...
#define ZTE_SPRD_SETTING_VOTER "ZTE_SPRD_SETTING_VOTER"
...
struct charger_policy {
	...
	struct zte_power_supply	*zte_usb_power_phy;
	struct zte_power_supply	*zte_ac_power_phy;
	struct zte_power_supply	*zte_hook_psy;
...
struct charger_manager {
	...
	struct zte_power_supply_desc zte_charger_psy_desc;
	struct zte_power_supply *zte_charger_psy;
...
#if IS_ENABLED(CONFIG_CHARGER_MANAGER) || defined(CONFIG_VENDOR_SQC_CHARGER)
