...
#endif
#ifndef __GENKSYMS__  /* 隐藏完整 mmu_notifier_mm 定义,保 register_shrinker KMI(零布局改动) */
...
/*
 * Compatibility shims: this MGLRU snapshot was written against the chromeos-5.4
 * tree, which carries a few helpers/APIs not present in this vendor tree.
 */
#ifndef arch_has_hw_pte_young
static inline bool arch_has_hw_pte_young(void)
{
	/* arm64 sets the access flag in hardware (ARMv8.1 HW AF) */
...
#define DEFINE_MAX_SEQ()						\
	unsigned long max_seq = READ_ONCE(lruvec_lrugen(lruvec)->max_seq)

#define DEFINE_MIN_SEQ()						\
	unsigned long min_seq[ANON_AND_FILE] = {			\
		READ_ONCE(lruvec_lrugen(lruvec)->min_seq[0]),		\
		READ_ONCE(lruvec_lrugen(lruvec)->min_seq[1]),		\
	}
...
{
	return lruvec_lrugen(lruvec)->max_seq - lruvec_lrugen(lruvec)->min_seq[type] + 1;
}
...
{
	struct lrugen *lrugen = lruvec_lrugen(lruvec);
...
{
	mm->lrugen = kzalloc(sizeof(struct lru_gen_mm_state), GFP_KERNEL);
	if (!mm->lrugen)
		return -ENOMEM;
...
static bool get_next_mm(struct mm_walk_args *args, struct mm_struct **iter)
{
	...
	while (!mm && mm_list->nodes[nid].iter != &mm_list->head) {
		mm = list_entry(mm_list->nodes[nid].iter, struct lru_gen_mm_state, list)->mm;
		mm_list->nodes[nid].iter = mm_list->nodes[nid].iter->next;
...
{
	/* chromeos-5.4 batched mmu-notifier young-clearing API is absent in this tree */
}
...
void lru_gen_scan_around(struct page_vma_mapped_walk *pvmw)
{
	...
	new_gen = lru_gen_from_seq(lruvec_lrugen(lruvec)->max_seq);
...
static bool fill_lru_gen_lists(struct lruvec *lruvec)
{
	...
	for_each_evictable_lru(lru) {
		...
		if (!lruvec_lrugen(lruvec)->enabled[type])
			continue;
...
static bool drain_lru_gen_lists(struct lruvec *lruvec)
{
	...
	for_each_gen_type_zone(gen, type, zone) {
		struct list_head *head = &lruvec_lrugen(lruvec)->lists[gen][type][zone];
...
/*
 * Number of kswapd threads to run on each NUMA node.  Controlled via the
 * vm.kswapd_threads sysctl; the default of 1 preserves the original,
 * single-kswapd-per-node behaviour.
 */
	...
	for_each_node_state(nid, N_MEMORY) {
		...
		if (cpumask_any_and(cpu_online_mask, mask) < nr_cpu_ids) {
			...
			for (i = 0; i < MAX_KSWAPD_THREADS; i++)
				if (pgdat->mkswapd[i])
					set_cpus_allowed_ptr(pgdat->mkswapd[i],
							     mask);
		}
	}
	return 0;
...
/*
 * Start the configured number of kswapd threads for a single node.  Threads
 * already running are left untouched, so this is safe to call again after the
 * vm.kswapd_threads sysctl has been increased.  Merged with MGLRU's per-node
 * lru_gen kswapd hook.
 */
...
/*
 * Stop all kswapd threads currently running for a single node.  Merged with
 * MGLRU's per-node lru_gen kswapd hook.
 */
...
/*
 * Bring the number of running kswapd threads per node in line with the current
 * value of the vm.kswapd_threads sysctl.
 */
static void update_kswapd_threads(void)
{
	int nid;

	if (kswapd_threads == kswapd_threads_current)
		return;

	for_each_node_state(nid, N_MEMORY) {
		pr_info("Updating kswapd threads on node %d to %d\n",
			nid, kswapd_threads);
		kswapd_per_node_stop(nid);
...
/*
 * sysctl handler for vm.kswapd_threads.  Clamps to [1, MAX_KSWAPD_THREADS] and
 * (re)starts the kswapd threads accordingly.
 */
