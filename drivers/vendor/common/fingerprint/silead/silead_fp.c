...
typedef enum _fp_spi_speet_t {
	...
	SPEED_25M = 25 * 1000 * 1000,
	SPEED_HIGH = SPEED_25M,
} fp_spi_speet_t;
...
static void sil_report_uevent(struct silfp_data *fp_dev, char *str)
{
	...
	if (fp_dev->nav_dev) {
		LOG_MSG_DEBUG(INFO_LOG, "%s fp_dev->nav_dev is not null!\n", __func__);
		kobject_uevent_env(&(fp_dev->nav_dev->dev.kobj), KOBJ_CHANGE, envp);
	} else {
		LOG_MSG_DEBUG(ERR_LOG, "%s fp_dev->nav_dev is null!\n", __func__);
	}
}
...
static void silfp_netlink_send(struct silfp_data *fp_dev, const int cmd)
{
	...
	LOG_MSG_DEBUG(DEBUG_LOG, "[%s] enter, send cmd %d\n", __func__, cmd);
	...
	*(char *)NLMSG_DATA(nlh) = cmd;
	...
	LOG_MSG_DEBUG(DEBUG_LOG, "[%s] sent, len=%d\n", __func__, ret);
}
...
static void silfp_netlink_recv(struct sk_buff *__skb)
{
	...
	if (skb->len >= NLMSG_SPACE(0)) {
		...
		if ((strlen(str) == strlen("FP")) && !strncmp(str, "FP", strlen(str))) {
			pid = nlh->nlmsg_pid;
			LOG_MSG_DEBUG(DEBUG_LOG, "[%s] PID=%d, msg=%s\n", __func__, pid, str);
		} else {
...
static void silfp_netlink_send(struct silfp_data *fp_dev, const int cmd)
{
	...
	LOG_MSG_DEBUG(DEBUG_LOG, "silfp_poll cmd %d\n", cmd);
			     ...
			     unsigned long event, void *data)
{
	...
	LOG_MSG_DEBUG(DEBUG_LOG, "[%s] enter, blank=0x%x\n", __func__, blank);

	switch (blank) {
	case FB_BLANK_UNBLANK:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] LCD ON\n", __func__);
		fp_dev->scr_off = 0;
		silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_ON);
		break;

	case FB_BLANK_POWERDOWN:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] LCD OFF\n", __func__);
		fp_dev->scr_off = 1;
		silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_OFF);
		break;
...
void silfp_irq_disable(struct silfp_data *fp_dev)
{
	...
	if (!fp_dev->irq_is_disable) {
		...
		LOG_MSG_DEBUG(INFO_LOG, "[%s] irq is disable\n", __func__);
	}
...
void silfp_irq_enable(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->irq_is_disable) {
		...
		LOG_MSG_DEBUG(INFO_LOG, "[%s] irq is enabled\n", __func__);
	}
	spin_unlock_irqrestore(&fp_dev->irq_lock, irqflags);
}
...
int silfp_irq_status(struct silfp_data *fp_dev)
{
	...
	/* this function is obsolete, for test purpose only.*/
	if (fp_dev->int_port) {
		LOG_MSG_DEBUG(DEBUG_LOG, "fp_dev->int_port=%d\n", gpio_get_value(fp_dev->int_port));
...
void silfp_work_func(struct work_struct *work)
{
	...
	LOG_MSG_DEBUG(DEBUG_LOG, "[%s] is running\n", __func__);
	silfp_netlink_send(fp_dev, SIFP_NETLINK_IRQ);
}
...
static int silfp_keyevent(struct silfp_data *fp_dev, struct fp_dev_key_t *pkey)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] pkey->value %d, pkey->flag %d\n", __func__, pkey->value, pkey->flag);
	if (IS_KEY_VALID(pkey->value)) {
		...
		case NAV_KEY_DOWN:
			...
			LOG_MSG_DEBUG(INFO_LOG, "%s nav down\n", __func__);
		break;

		case NAV_KEY_UP:
			sil_report_uevent(fp_dev, FP_NAV_UP);
			LOG_MSG_DEBUG(INFO_LOG, "%s nav up\n", __func__);
		break;

		case NAV_KEY_RIGHT:
			/*sil_report_uevent(fp_dev, FP_NAV_RIGHT);*/
			LOG_MSG_DEBUG(INFO_LOG, "%s nav right\n", __func__);
		break;

		case NAV_KEY_LEFT:
			/*sil_report_uevent(fp_dev, FP_NAV_LEFT);*/
			LOG_MSG_DEBUG(INFO_LOG, "%s nav left\n", __func__);
		break;

		default:
			LOG_MSG_DEBUG(ERR_LOG, "%s unknown nav event: %d\n", __func__, pkey->value);
		break;
		}
	}
	ret = 0;
		...
		if (keymap[i].key_orig == pkey->value) {
			...
			if (keymap[i].key_new != KEY_RESERVED) {
				if (pkey->flag == NAV_KEY_FLAG_CLICK) {
					...
					LOG_MSG_DEBUG(INFO_LOG, "[%s] input_report_key2 %d\n", __func__, keymap[i].key_new);
...
static int silfp_proc_create_node(struct silfp_data *fp_dev)
{
	...
	LOG_MSG_DEBUG(DEBUG_LOG, "[%s] exit\n", __func__);
	return 0;
...
static void silfp_wakelock_ctl(struct silfp_data *fp_dev, unsigned char lock)
{
	LOG_MSG_DEBUG(DEBUG_LOG, "[%s] enter\n", __func__);
	if (lock) {
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] wake_lock_timeout\n", __func__);
		wake_lock_timeout(&fp_dev->wakelock_hal, 10 * HZ);
	} else {
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] wake_unlock\n", __func__);
...
static long silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	unsigned char delay, level, lock, flag_avdd, feature;
	...
	switch (cmd) {
	case SIFP_IOC_INIT:
		LOG_MSG_DEBUG(INFO_LOG, "[%s] SIFP_IOC_INIT\n", __func__);
	...
	case SIFP_IOC_RESET:
		delay = RESET_TIME;
		LOG_MSG_DEBUG(INFO_LOG, "[%s] SIFP_IOC_RESET\n", __func__);
		if (arg) {
	...
	case SIFP_IOC_ENABLE_IRQ:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_ENABLE_IRQ\n", __func__);
		silfp_irq_enable(fp_dev);
		break;

	case SIFP_IOC_DISABLE_IRQ:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_DISABLE_IRQ\n", __func__);
		silfp_irq_disable(fp_dev);
		break;

	case SIFP_IOC_CLR_IRQ:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_CLR_IRQ\n", __func__);
		reinit_completion(&fp_dev->done);
		break;

	case SIFP_IOC_WAIT_IRQ:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_WAIT_IRQ\n", __func__);
		retval = __put_user(silfp_wait_irq(fp_dev) ? 1 : 0, (__u8 __user *)arg);
		break;

	case SIFP_IOC_IRQ_STATUS:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_IRQ_STATUS\n", __func__);
	...
	case SIFP_IOC_SCR_STATUS:
		if (arg) {
			LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_SCR_STATUS put v = %d\n", __func__, (u8)(!fp_dev->scr_off));
			retval = __put_user((u8)(!fp_dev->scr_off), (__u8 __user *)arg);
		} else {
			LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_SCR_STATUS send v = %d\n", __func__, (u8)(!fp_dev->scr_off));
	...
	case SIFP_IOC_ACQ_SPI:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_ACQ_SPI\n", __func__);
		silfp_set_spi(fp_dev, true);
		break;

	case SIFP_IOC_RLS_SPI:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_RLS_SPI\n", __func__);
		silfp_set_spi(fp_dev, false);
		break;

	case SIFP_IOC_PKG_SIZE:
		if (arg) {
			LOG_MSG_DEBUG(DEBUG_LOG, "[IOC_PKG_SIZE] %d\n", (u8)(PKG_SIZE));
			retval = __put_user((u8)(PKG_SIZE), (__u8 __user *)arg);
		}
		break;

	case SIFP_IOC_DBG_LEVEL:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_DBG_LEVEL\n", __func__);
		if (arg) {
			level = 0;
	...
	case SIFP_IOC_WAKELOCK:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_WAKELOCK\n", __func__);
		if (arg) {
			lock = 0;
			...
			LOG_MSG_DEBUG(DEBUG_LOG, "silfp_wakelock_ctl\n");
			silfp_wakelock_ctl(fp_dev, lock);
		}
		break;

	case SIFP_IOC_PWDN:
		LOG_MSG_DEBUG(DEBUG_LOG, "[%s] SIFP_IOC_PWDN\n", __func__);
		if (arg) {
			flag_avdd = 0;
...
int  silfp_touch_event_handler(struct fp_dev_touch_info *tp_info)
{
	...
	LOG_MSG_DEBUG(DEBUG_LOG, "tp_info %d, %d, %d, %d\n",
...
static int silfp_open(struct inode *inode, struct file *filp)
{
	...
	list_for_each_entry(fp_dev, &device_list, device_entry) {
		if (fp_dev->devt == inode->i_rdev) {
			LOG_MSG_DEBUG(INFO_LOG, "sileadfp_dev Found!\n");
			status = 0;
			break;
		}
	}
	...
	if (dnode) {
		LOG_MSG_DEBUG(INFO_LOG, "fp-nav device node found!\n");
		fp_dev->nav_dev = of_find_device_by_node(dnode);
		if (fp_dev->nav_dev)
		{
			LOG_MSG_DEBUG(INFO_LOG, "fp-nav device uevent found!\n");
		} else {
			LOG_MSG_DEBUG(ERR_LOG, "fp-nav device uevent not found!\n");
		}
	} else {
		LOG_MSG_DEBUG(ERR_LOG, "fp-nav device node not found!\n");
	}
	...
	if (status == 0) {
		...
		LOG_MSG_DEBUG(INFO_LOG, "Succeed to open sileadfp_dev!\n");
	} else {
...
#endif
{
	...
	fp_dev->power_type        = 0;
	fp_dev->fp_reg            = NULL;
	fp_dev->power_voltage     = 2800000;/*default fp voltage*/
...
int silfp_dev_init(void)
{
	...
	*/
	...
	LOG_MSG_DEBUG(INFO_LOG, "----silead spi_register_driver ok----status : %d\n", status);
#elif defined(USE_PLATFORM_BUS)
	status = platform_driver_register(&silfp_driver);
	LOG_MSG_DEBUG(INFO_LOG, "----silead platform_driver_register ok----status : %d\n", status);
#endif
	if (status < 0) {
		class_destroy(silfp_class);
		unregister_chrdev(FP_DEV_MAJOR, silfp_driver.driver.name);
		LOG_MSG_DEBUG(ERR_LOG, "[%s] register_driver fail ret=%d.\n", __func__, status);
		return status;
	}
	silfp_wq = create_singlethread_workqueue("silfp_wq");
	LOG_MSG_DEBUG(INFO_LOG, "[%s]  dev_init success!\n", __func__);
	return status;
}
