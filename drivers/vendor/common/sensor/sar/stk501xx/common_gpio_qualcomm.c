/*
 *  common_gpio_qualcomm.c - Linux kernel modules for sensortek stk5xxx
...
int register_gpio_irq(stk_gpio_info *gpio_info)
{
    ...
    {
        case TRIGGER_RISING:
            err = request_threaded_irq(irq, stk_gpio_irq_handler, NULL, IRQF_TRIGGER_RISING, \
                                          linux_gpio_mgr[gpio_idx].gpio_info->device_name, &linux_gpio_mgr[gpio_idx]);
            break;

        case TRIGGER_FALLING:
            err = request_threaded_irq(irq, stk_gpio_irq_handler, NULL, IRQF_TRIGGER_FALLING, \
                                          linux_gpio_mgr[gpio_idx].gpio_info->device_name, &linux_gpio_mgr[gpio_idx]);
            break;

        case TRIGGER_HIGH:
        case TRIGGER_LOW:
            err = request_threaded_irq(irq, stk_gpio_irq_handler, NULL, IRQF_TRIGGER_LOW, \
                                          linux_gpio_mgr[gpio_idx].gpio_info->device_name, &linux_gpio_mgr[gpio_idx]);
            break;
    ...
    if (err < 0)
    {
        printk(KERN_WARNING "%s: request_threaded_irq(%d) failed for (%d)\n", __func__, irq, err);
