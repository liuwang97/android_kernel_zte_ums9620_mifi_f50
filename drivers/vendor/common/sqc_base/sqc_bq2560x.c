...
do {											\
	if (info->hw_mode == BQ25601_MASTER) {					\
		pr_err("[bq2560x-MASTER]:%s:" fmt, __func__, ##__VA_ARGS__);	\
	} else { \
		pr_err("[bq2560x-SLAVE]:%s:" fmt, __func__, ##__VA_ARGS__);\
	} \
} while (0);
...
do {											\
	if (info->hw_mode == BQ25601_MASTER) {					\
		pr_info("[bq2560x-MASTER]:%s:" fmt, __func__, ##__VA_ARGS__);	\
	} else {		\
		pr_info("[bq2560x-SLAVE]:%s:" fmt, __func__, ##__VA_ARGS__); \
	} \
} while (0);
...
do {											\
	if (info->hw_mode == BQ25601_MASTER)	{					\
		pr_debug("[bq2560x-MASTER]:%s:" fmt, __func__, ##__VA_ARGS__);	\
	} else {			\
		pr_debug("[bq2560x-SLAVE]:%s:" fmt, __func__, ##__VA_ARGS__); \
	} \
} while (0);
...
static void bq2560x_print_regs(struct bq2560x_charger_info *info)
{
	...
	for (i = 0; i < ARRAY_SIZE(value); i++) {
		...
		if (i == ARRAY_SIZE(value) - 1) {
			bq_info("####### bq25601: 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
...
static int bq2560x_charger_dumper_reg(struct bq2560x_charger_info *info)
{
	...
	bq_info("bq2560x: charging[%d], fcv[%d], usb_icl[%d], fcc[%d], topoff[%d], rechg_volt[%d]",
...
static int bq2560x_charger_tuning_vindpm_insert(struct bq2560x_charger_info *info)
{
	...
	if (vchg > info->vindpm_value) {
		...
		bq_info("%s: vchg %d, batt_vol_uv=%d, now vindpm %d, set vindpm to %d\n",
...
static int sqc_mp_set_enbale_chging(void *arg, unsigned int en)
{
	...
	if (en) {
		...
		bq2560x_charger_set_vindpm(info, 4700);
		bq2560x_charger_dumper_reg(info);
		if (info->gpiod)
...
static struct sqc_pmic_chg_ops bq2560x_sqc_chg_ops = {
	...
	.get_chg_type = sqc_mp_get_enbale_chging,
...
*/
static void bq2560x_get_chip_vendor_id(struct bq2560x_charger_info *info)
{
	u8 reg_value = 0;
	u8 pn = 0;
	int ret = -1;
	...
	bq_info("%s reg=0x%x, value=0x%x pn=0x%x sub_id=0x%x\n",
		__func__, BQ2560X_REG_0B, reg_value, pn, sub_id);
		...
		const struct i2c_device_id *id)
{
	...
	if (of_device_is_compatible(regmap_np->parent, "sprd,sc2730"))
		...
		bq_err("failed to get charger_pd mask\n");
		return -ENODEV;
	}
