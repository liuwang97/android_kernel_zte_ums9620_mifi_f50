...
static void charge_policy_ktime_reset(struct charger_policy_info *policy_info)
{
	...
	alarm_cancel(&policy_info->timeout_timer);
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	OF_READ_PROPERTY_STRINGS(policy_info->config_prop.zte_interface_phy_name,
						...
						bool battery_in_enable, bool usb_in_enable)
{
	...
	/*get info*/
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	rc = zte_charger_policy_get_prop_by_name(policy_info->config_prop.zte_interface_phy_name,
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
		rc = zte_charger_policy_set_prop_by_name(policy_info->config_prop.zte_interface_phy_name,
							...
							ktime_t now)
{
	...
	alarm_forward(alarm, now, ms_to_ktime(policy_info->timeout_interval));
