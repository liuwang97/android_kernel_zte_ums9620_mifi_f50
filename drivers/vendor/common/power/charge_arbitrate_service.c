...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	const char	*zte_interface_phy_name;
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
#ifdef KERNEL_ABOVE_4_1_0
	struct zte_power_supply		*zte_cas_psy_pointer;
#else
	struct zte_power_supply		zte_cas_psy;
#endif
#endif
...
#define UI_SOC_FULL							100
#define HEARTBEAT_SECOND					300
...
extern struct cas_status_list status_list[];

static const char * const charge_status_text[] = {
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
static int zte_power_supply_set_prop_by_name(const char *name, enum zte_power_supply_property psp, int data)
{
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
		zte_power_supply_set_prop_by_name(cas_info->config_prop.zte_interface_phy_name,
...
#ifdef ZTE_FEATURE_BATTERY_AGING_FULL
	if (!cas_info->debug_prop.age) {
		pr_alarm("feature force aging to 100\n");
		i = 0;
	} else {
		pr_alarm("debug aging i=%d\n", i);
		return i;
	}
#else
	i = cas_info->config_prop.force_full_aging ? 0 : i;
...
static int charge_arbitrate_get_charge_status(struct charge_arbitrate_info *cas_info)
{
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.zte_battery_phy_name,
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.zte_interface_phy_name,
			"zte-interface-phy-name", retval, "zte_interface");
#endif
				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		pval->intval = cas_info->debug_prop.charge_status;
		break;
	case POWER_SUPPLY_PROP_MODEL_NAME:
		pval->strval = status_list[cas_info->status].status_name;
		break;
	case POWER_SUPPLY_PROP_CALIBRATE:
		pval->intval = cas_info->debug_prop.age;
		break;
	default:
				  ...
				  const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		break;
#if !defined(CONFIG_ZTE_POWER_SUPPLY_COMMON)
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
static int zte_cas_psy_get_property(struct zte_power_supply *psy,
				       enum zte_power_supply_property psp,
				       union power_supply_propval *pval)
{
	struct charge_arbitrate_info *cas_info = zte_power_supply_get_drvdata(psy);
	int rc = 0;
...
static int zte_cas_psy_set_property(struct zte_power_supply *psy,
				  enum zte_power_supply_property psp,
				  const union power_supply_propval *pval)
{
	struct charge_arbitrate_info *cas_info = zte_power_supply_get_drvdata(psy);
...
#ifdef KERNEL_ABOVE_4_1_0
	zte_power_supply_changed(cas_info->zte_cas_psy_pointer);
#else
	zte_power_supply_changed(&cas_info->zte_cas_psy);
#endif

	return rc;
}

static int zte_cas_property_is_writeable(struct zte_power_supply *psy,
						enum zte_power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_CAPACITY_RAW_ZTE:
...
static void zte_cas_external_power_changed(struct zte_power_supply *psy)
{
	pr_debug("zte power supply changed\n");
}


static enum zte_power_supply_property zte_cas_psy_props[] = {
	POWER_SUPPLY_PROP_CAPACITY_RAW_ZTE,
};

#ifdef KERNEL_ABOVE_4_1_0
static const struct zte_power_supply_desc zte_cas_psy_desc = {
	.name = "zte_cas",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
	.properties = zte_cas_psy_props,
	.num_properties = ARRAY_SIZE(zte_cas_psy_props),
	.get_property = zte_cas_psy_get_property,
	.set_property = zte_cas_psy_set_property,
	.external_power_changed = zte_cas_external_power_changed,
	.property_is_writeable = zte_cas_property_is_writeable,
};
#endif
#endif
...
static int charge_arbitrate_debug_battery_info(struct charge_arbitrate_info *cas_info)
{
	...
	if ((cas_info->status_pre == cas_info->status)
			&& (cas_info->charge_status_pre == cas_info->charge_status)
			&& ((ts.tv_sec - cas_info->time_pre) < HEARTBEAT_SECOND)) {
		cas_info->logger_limit = false;
		return 0;
	} else {
		cas_info->logger_limit = true;
	}
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	rc = zte_power_supply_get_property(cas_info->zte_battery_psy,
				POWER_SUPPLY_PROP_CAPACITY_RAW_ZTE, &capacity_raw);
	if (rc < 0) {
		pr_alarm("Failed to get CAPACITY_RAW zte property rc=%d\n", rc);
	}
#else
	rc = power_supply_get_property(cas_info->battery_psy,
				POWER_SUPPLY_PROP_CAPACITY_RAW_ZTE, &capacity_raw);
	...
	pr_alarm("status: %s[%s], cap: %d, cap_raw: %d, Vocv: %d, Vnow: %d, Inow: %d, temp: %d\n",
						status_list[cas_info->status].status_name,
						charge_status_text[cas_info->charge_status],
						capacity_val.intval,
						capacity_raw.intval,
						voltage_ocv.intval,
						voltage_now.intval,
	...
	cas_info->time_pre = ts.tv_sec;

	cas_info->status_pre = cas_info->status;
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	rc = zte_power_supply_get_property(cas_info->zte_interface_psy,
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
		rc = zte_power_supply_set_property(cas_info->zte_interface_psy,
					POWER_SUPPLY_PROP_INPUT_SUSPEND, &input_suspend);
#else
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
		cas_info->run_prop.time_begin = ktime_to_timespec(ktime_get_boottime());
#else
...
static int charge_arbitrate_c3_status_chargefull(struct charge_arbitrate_info *cas_info)
{
	...
	if (battery_capacity.intval == UI_SOC_FULL) {
		...
		if (rc == TEMP_SAFE_MODE_NORMAL) {
			...
			pr_alarm("Terminated in JEITA, ui_soc %d\n", battery_capacity.intval);
		}
	}
...
static int charge_arbitrate_r1_p1_status_chargefull(struct charge_arbitrate_info *cas_info)
{
	...
	union power_supply_propval voltage_ocv = {0, };
	union power_supply_propval charge_status = {0, };
	int usb_online = 0, dc_online = 0, rc = 0;
	...
	charge_status.intval = charge_arbitrate_get_charge_status(cas_info);

	rc = power_supply_get_prop_by_name("usb",
				POWER_SUPPLY_PROP_PRESENT, &usb_online);
	if (rc < 0) {
		pr_alarm("Failed to get usb present property rc=%d\n", rc);
	}

	rc = power_supply_get_prop_by_name("dc",
				POWER_SUPPLY_PROP_PRESENT, &dc_online);
	if (rc < 0) {
		pr_alarm("Failed to get dc present property rc=%d\n", rc);
	}

	switch (charge_status.intval) {
	case POWER_SUPPLY_STATUS_DISCHARGING:
	case POWER_SUPPLY_STATUS_NOT_CHARGING:
		if (cas_info->logger_limit)
			pr_alarm("status %d, usb_online %d, dc_online %d, abnormal_chgfull %d\n",
				charge_status.intval, usb_online, dc_online, cas_info->abnormal_chgfull);

		if ((!usb_online && !dc_online) || (cas_info->abnormal_chgfull)) {
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		cas_info->abnormal_chgfull = false;
	...
	case POWER_SUPPLY_STATUS_FULL:/*original process, do nothing*/
		cas_info->abnormal_chgfull = true;
		if (cas_info->logger_limit)
...
static int charge_arbitrate_turnoff_state_machine(struct charge_arbitrate_info *cas_info)
{
	int usb_online = 0, dc_online = 0, rc = 0, age_index = 0;
	...
	if (usb_online || dc_online) {
		pr_alarm("Discharge Exit:  because usb/dc is online\n");
		return 0;
	}
	...
	/*restore usb icl and default arg*/
	charge_arbitrate_set_input_suspend(cas_info, false);
...
static int charge_arbitrate_battery_notifier_handler(struct charge_arbitrate_info *cas_info)
{
	...
	cas_info->charge_status = charge_arbitrate_get_charge_status(cas_info);
	...
	if (cas_info->force_charger_full) {
		cas_info->charge_status = POWER_SUPPLY_STATUS_FULL;
	}
	...
	if (cas_info->disable_cas) {
		...
		cas_info->status = CHARGE_STATUS_CHARGE_START_C1;
		cas_info->charge_status = POWER_SUPPLY_STATUS_DISCHARGING;
		charge_arbitrate_ktime_disable(cas_info);
	}

	switch (cas_info->charge_status) {
	case POWER_SUPPLY_STATUS_CHARGING:
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
		cas_info->zte_battery_psy = zte_power_supply_get_by_name(cas_info->config_prop.zte_battery_phy_name);
		if (!cas_info->zte_battery_psy) {
			pr_alarm("Get zte_battery psy failed!!!!\n");
			goto zte_battery_psy_failed;
		}

	cas_info->zte_interface_psy = zte_power_supply_get_by_name(cas_info->config_prop.zte_interface_phy_name);
	if (!cas_info->zte_interface_psy) {
		pr_alarm("Get zte_interface psy failed!!!!\n");
		goto zte_interface_psy_failed;
	}
#endif
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	cas_info->zte_cas_psy_pointer = zte_devm_power_supply_register(cas_info->dev, &zte_cas_psy_desc,
			&cas_psy_cfg);
	if (IS_ERR(cas_info->zte_cas_psy_pointer)) {
		pr_alarm("failed to register zte_cas_psy rc = %ld\n",
				PTR_ERR(cas_info->zte_cas_psy_pointer));
		goto register_zte_power_supply_failed;
	}
#endif
#else
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	memset(&cas_info->zte_cas_psy, 0, sizeof(struct zte_power_supply));
	cas_info->zte_cas_psy.name = "zte_cas";
	cas_info->zte_cas_psy.type = POWER_SUPPLY_TYPE_BMS;
	cas_info->zte_cas_psy.properties = zte_cas_psy_props;
	cas_info->zte_cas_psy.num_properties = ARRAY_SIZE(zte_cas_psy_props);
	cas_info->zte_cas_psy.get_property = zte_cas_psy_get_property;
	cas_info->zte_cas_psy.set_property = zte_cas_psy_set_property;
	cas_info->zte_cas_psy.external_power_changed = zte_cas_external_power_changed;
	cas_info->zte_cas_psy.property_is_writeable = zte_cas_property_is_writeable;
	cas_info->zte_cas_psy.drv_data = cas_info;
	cas_info->zte_cas_psy.of_node = NULL;
	cas_info->zte_cas_psy.supplied_to = NULL;
	cas_info->zte_cas_psy.num_supplicants = 0;

	rc = zte_power_supply_register(cas_info->dev, &cas_info->zte_cas_psy);
	if (rc < 0) {
		pr_alarm("failed to register cas_psy rc = %d\n", rc);
		goto register_zte_power_supply_failed;
	}
...
register_zte_power_supply_failed:
	pr_alarm("register zte_power_supply failed!!!\n");
register_power_supply_failed:
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
#ifdef KERNEL_ABOVE_4_1_0
	zte_power_supply_put(cas_info->zte_interface_psy);
#endif
	cas_info->zte_interface_psy = NULL;
#else
#ifdef KERNEL_ABOVE_4_1_0
...
#endif
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
zte_interface_psy_failed:
#ifdef KERNEL_ABOVE_4_1_0
	zte_power_supply_put(cas_info->zte_battery_psy);

#endif
	cas_info->zte_battery_psy = NULL;

zte_battery_psy_failed:
#ifdef KERNEL_ABOVE_4_1_0
		power_supply_put(cas_info->interface_psy);
#endif
		cas_info->interface_psy = NULL;
...
static int charge_arbitrate_probe(struct platform_device *pdev)
{
	...
	pr_alarm("charge Arbitrate driver probe exit, in pv mode\n");
	return 0;
#else
...
#ifdef CONFIG_ZTE_POWER_SUPPLY_COMMON
	queue_delayed_work(cas_info->cas_probe_wq, &cas_info->cas_probe_work, msecs_to_jiffies(10000));
#else
