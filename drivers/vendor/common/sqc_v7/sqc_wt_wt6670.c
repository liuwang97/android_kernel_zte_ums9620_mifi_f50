...
int __attribute__ ((weak))  sqc_get_sdp_auth_status(void)
{
	return 0;
}
...
static int wt6670_i2c_read(struct i2c_client *client, u16 addr, u16 len, void *rxbuf)
{
	...
	if (ret < 0) {
		WT6670_ERROR("wt6670_i2c_read addr 0x%04X failed ret=%d\n", addr, ret);
		wt6670_i2c_comm_unlock();
		return ret;
	}
...
static void wt6670_vote_fcc_during_detect(int enable)
{
	union power_supply_propval val = {0, };

	if (enable) {
		WT6670_INFO("WT6670_Vote 500mA recheck_flag:%d\n", gWt6670->recheck_flag);
		val.intval = 500000;
		sqc_set_property(POWER_SUPPLY_PROP_HW_CURRENT_MAX, &val);
	} else {
		WT6670_INFO("WT6670_Vote clean recheck_flag:%d\n", gWt6670->recheck_flag);
		val.intval = -1;
		sqc_set_property(POWER_SUPPLY_PROP_HW_CURRENT_MAX, &val);
	}
}
...
static void wt6670_int_handle_work(struct work_struct *work)
{
	...
	WT6670_INFO("### Detect is finished recheck_flag:%d-----\n", gWt6670->recheck_flag);
	...
	/* Set current to 500mA before QC3+ detect */
	if (gWt6670->recheck_flag == 0) {
		wt6670_vote_fcc_during_detect(1);
	}
	...
	if (chg_type == WT6670_VAL_CHG_TYPE_UNKNOWN) {
		...
		if (gWt6670->recheck_flag == 0) {
			...
			WT6670_INFO("### recheck chg type delay 2500ms, recheck_flag: 1\n");
	...
	WT6670_INFO("notify status changed %d, recheck_flag:%d\n",
		(chg_type != WT6670_VAL_CHG_TYPE_UNKNOWN), gWt6670->recheck_flag);
	...
	if ((battery_get_boot_mode() == RECOVERY_BOOT) || (battery_get_boot_mode() == META_BOOT)) {
		wt6670_inform_psy_changed(gWt6670);
	}

	/* Clean Vote after QC3+ detect */
	if (gWt6670->recheck_flag == 3) {
		wt6670_vote_fcc_during_detect(0);
	}
...
static int wt6670_status_init(void)
{
	...
	gWt6670->chg_type = WT6670_VAL_CHG_TYPE_UNKNOWN;
	flush_delayed_work(&gWt6670->int_work);
	cancel_delayed_work_sync(&gWt6670->int_work);
	flush_delayed_work(&gWt6670->det_work);
	cancel_delayed_work_sync(&gWt6670->det_work);
	gWt6670->recheck_flag = 0;
...
static void wt6670_det_handle_work(struct work_struct *work)
{
	...
	if (gWt6670->chg_type == WT6670_VAL_CHG_TYPE_SDP) {
		if ((sqc_get_sdp_auth_status() != 0) || (battery_get_boot_mode() == META_BOOT)) {
			gWt6670->recheck_flag = 3;
			wt6670_vote_fcc_during_detect(0);
			WT6670_INFO("### recheck canceled(sdp), recheck_flag: 3, bootmode: %d\n", get_boot_mode());
			return;
		} else {
			Charger_Detect_Init();
			WT6670_INFO("### sdp auth failed, rechecking\n");
		}
	}
...
int wt6670_chg_id_get(char *val, const void *arg)
{
	if (!gWt6670) {
		WT6670_ERROR("gWt6670 is null\n");
		return snprintf(val, PAGE_SIZE, "%d", -1);
	}

	return snprintf(val, PAGE_SIZE, "%d", !!gWt6670->FirmwareVer);
}

static struct zte_misc_ops protocol_chg_id_node = {
	.node_name = "protocol_chg_id",
	.set = NULL,
	.get = wt6670_chg_id_get,
	.free = NULL,
	.arg = NULL,
};
...
static int wt6670_probe_work(struct work_struct *work)
{
	...
	zte_misc_register_callback(&protocol_chg_id_node, gWt6670);
