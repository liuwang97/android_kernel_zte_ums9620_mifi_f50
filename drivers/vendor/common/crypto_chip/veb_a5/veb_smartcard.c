...
typedef struct {
    ...
    u32 outlen;
    u32 outbuf;
} veb_smartcard_com_data_t;


/*********************************************************************************************
*********************************************************************************************/
int veb_smartcard_cmd(struct spi_device *spi, smart_cmd_t stCmd, unsigned char *pDataIn, unsigned int uInLen, unsigned char *pDataOut, unsigned int *puOutLen)
{
...
int veb_smartcard_cmd_read(struct spi_device *spi, smart_cmd_t *cmd, char *buf, unsigned int *outlen)
{
    ...
    #endif
    if(0 == memcmp(init_apdu, cmd->cmd, 4))
    {
...
long veb_smartcard_ioctl(veb_private_data_t *priv, unsigned int cmd, unsigned long arg)
{
    ...
    else if (VEB_DATA_MMAP == data.type)
    {
        if (copy_to_user((void __user *)data.outlen, (char *)&uOutLen, sizeof(uOutLen)))
        {
...
long veb_smartcard_compat_ioctl_read(veb_private_data_t *priv, unsigned int cmd, unsigned long arg)
{
    veb_smartcard_data_t data;
    long ret = VEB_OK;
    char *buf = NULL;
    veb_smartcard_com_data_t __user * p_com_data =NULL;
    compat_uptr_t p;
    int err = 0;
    ...
    p_com_data = (veb_smartcard_com_data_t __user *)arg;
    ...
    err |= get_user(p, &(p_com_data->outlen));
    data.outlen = compat_ptr(p);
...
long veb_smartcard_compat_ioctl_exchange(veb_private_data_t *priv, unsigned int cmd, unsigned long arg)
{
    veb_smartcard_data_t d;
    long ret = VEB_OK;
    unsigned char *buf = NULL;
    ...
    memset(&d, 0, sizeof(d));
    if(copy_from_user(&d, (void __user *)arg, sizeof(smart_cmd_t)))
    {
        VEB_ERR("copy from user failed!\n");
        return VEB_ERR_COPY_FROM_USER;
    }
    p_com_data = (veb_smartcard_com_data_t __user *)arg;
    //err = get_user(data.cmd, &(p_com_data->cmd));
    err |= get_user(d.type, &(p_com_data->type));
    err |= get_user(d.inlen, &(p_com_data->inlen));
    err |= get_user(p, &(p_com_data->inbuf));
    d.inbuf = compat_ptr(p);
    err |= get_user(p, &(p_com_data->outlen));
    d.outlen = compat_ptr(p);
    err |= get_user(p, &(p_com_data->outbuf));
    d.outbuf = compat_ptr(p);
    if(err)
    {
        VEB_ERR("get user failed!\n");
        return VEB_ERR_COPY_FROM_USER;
