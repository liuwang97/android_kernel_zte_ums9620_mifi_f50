...
int check_sw(unsigned char *sw)
{
    ...
    if ((sw[1] != 0x90) || (sw[0] != 0))
    {
        ...
        else if((sw[1] == 0x00) && (sw[0] == 0x60))
        {
            ...
            return VEB_ERR_USERKEY_EMPTY;
        }
        else if((sw[1] == 0x90) && (sw[0] == 0x86))
        {
            VEB_ERR("sm2 sign check fail \n");
            return VEB_ERR_SM2_SIGNCHECK;
        }
        ...
        else if (0x6F == sw[1])
        {
            ...
            if (sw[0] == 0x01)  strcpy(check_type, "AES");
            else if (sw[0] == 0x02)  strcpy(check_type, "SM4");
            else if (sw[0] == 0x04)  strcpy(check_type, "SM3");
            else if (sw[0] == 0x08)  strcpy(check_type, "SM2");
            else if (sw[0] == 0x10)  strcpy(check_type, "RandNum");
            VEB_ERR("self checking %s error \n", check_type);
            return VEB_ERR_SELF_CHECKING;
        }
        else
        {
            VEB_ERR("unknow err, reset a5 ! \n");
            veb_spi_reset();
            return VEB_ERR_CHECK_SW_ERR;
        }
    }
...
int veb_sym_crypto(struct spi_device *spi, cmd_t *cmd, unsigned char *buf,
        unsigned char *IV, int len)
{
    ...
    buf_tmp = kmalloc(len + 16 + 4 + 2, GFP_KERNEL);
    if (buf_tmp == NULL) {
        VEB_ERR("buf malloc fail \n");
        return VEB_ERR_MALLOC;
    }
...
int veb_sym_duplex_crypto(struct spi_device *spi, cmd_t *cmd, unsigned char *buf,
        unsigned char *IV, int len)
{
    ...
    tmp_xfer = (unsigned char*)kmalloc((XFER_BLOCK_SIZE + 4) * count, GFP_KERNEL);
    if (tmp_xfer == NULL)
    {
    ...
    if (cmd->cmd[0] == VEB_IC_CLA_CRC)
    {
        ...
        tmp = tmp_xfer;
        for (i = 0; i < count; i++) // blind crc data
        {
        ...
        veb_key_info *veb_key, unsigned char *IV, int len)
{
    ...
    buf_tmp = kmalloc(len + AES_256_KEY_LEN + 16 + 4 + 2, GFP_KERNEL);
    if (buf_tmp == NULL) {
        VEB_ERR("buf malloc fail \n");
        return VEB_ERR_MALLOC;
    }
