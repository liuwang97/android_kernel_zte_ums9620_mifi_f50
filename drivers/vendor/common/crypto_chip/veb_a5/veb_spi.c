...
int veb_sprd_spi_controller_config(struct spi_device *spi)
{
    veb_private_data_t *priv = spi_get_drvdata(spi);
    int ret = VEB_OK;
    ...
    // if ((priv->pinctrl_state0 == NULL) ||
    //     (priv->pinctrl_state1 == NULL) ||
    //     (priv->p == NULL)) {
    //     VEB_ERR("%s pinctrl_state NULL\n", __func__);
    //     ret = VEB_ERROR;
    //     return ret;
    // }

    // ret = pinctrl_select_state(priv->p, priv->pinctrl_state0);
    // if (ret < 0) {
    //     VEB_ERR("veb_ap_spi2 pinctrl select error %d\n", ret);
    // }
    // ret = pinctrl_select_state(priv->p, priv->pinctrl_state1);
    // if (ret < 0) {
    //     VEB_ERR("veb_pscp_spi0 pinctrl select error %d\n", ret);
    // }

    return ret;
}

int veb_sprd_spi_controller_restore(struct spi_device *spi)
{
    veb_private_data_t *priv = spi_get_drvdata(spi);
    int ret = VEB_OK;
    ...
    // if ((priv->pinctrl_state2 == NULL) ||
    //     (priv->pinctrl_state3 == NULL) ||
    //     (priv->p == NULL)) {
    //     VEB_ERR("%s pinctrl_state NULL\n", __func__);
    //     ret = VEB_ERROR;
    //     return ret;
    ...
    // ret = pinctrl_select_state(priv->p, priv->pinctrl_state2);
    // if (ret < 0) {
    //     VEB_ERR("nfc_ap_spi2 pinctrl select error %d\n", ret);
    // }
    // ret = pinctrl_select_state(priv->p, priv->pinctrl_state3);
    // if (ret < 0) {
    //     VEB_ERR("nfc_pscp_spi0 pinctrl select error %d\n", ret);
    // }

    return ret;
}

int veb_spi_sync(struct spi_device *spi, struct spi_message *p)
{
    // struct spi_controller *spi_ctrl = spi->controller;
    int ret = VEB_OK;

    // mutex_lock(&(spi_ctrl->controller_lock_mutex));
    // ret = veb_sprd_spi_controller_config(spi);
    ret += spi_sync(spi, p);
    // ret += veb_sprd_spi_controller_restore(spi);
    // mutex_unlock(&(spi_ctrl->controller_lock_mutex));

    return ret;
}

// int veb_sprd_pinctrl_parse(struct spi_device *spi)
// {
//     veb_private_data_t *priv = spi_get_drvdata(spi);
//     int ret = VEB_OK;
...
//     priv->p = devm_pinctrl_get(&spi->dev);
//     priv->pinctrl_state0 = pinctrl_lookup_state(priv->p, "veb_ap_spi2");
//     priv->pinctrl_state1 = pinctrl_lookup_state(priv->p, "veb_pscp_spi0");
//     priv->pinctrl_state2 = pinctrl_lookup_state(priv->p, "nfc_ap_spi2");
//     priv->pinctrl_state3 = pinctrl_lookup_state(priv->p, "nfc_pscp_spi0");
//     return ret;
// }
...
static int veb_a5_spi_probe(struct spi_device *spi)
{
...
#else
    spi->mode = spi->mode|SPI_MODE_3;  // MUST BE THIS MODE UNDER SPRD
#endif
    ...
    // ret = veb_sprd_pinctrl_parse(spi);
    // if (ret < 0) {
    //     veb_platform_exit(spi);
    //     free_pages((unsigned long)priv->mmap_addr, get_order(VEB_MMAP_SIZE));
    //     kfree(priv);
    //     VEB_ERR("veb_a5 spi pinctrl setup error!\n");
    //     return ret;
    // }
