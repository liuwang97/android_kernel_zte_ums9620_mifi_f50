...
#define DEBUG(fmt, arg...)  printk(KERN_INFO "--atsha204a-- " fmt "\n", ##arg);
...
{
    atsha204a_sysdata_t *sha204_sysdata = (atsha204a_sysdata_t *)dev_get_drvdata(dev);
...
static int check_chip_sn(struct i2c_client *client)
{
    u8 sn[SHA204_SN_SIZE];
    int ret = 0;

    if (0 > atsha204a_read_sn(client, sn))
        return -EIO;

    DEBUG("%s sn<0 1 8> %02x %02x %02x\n", __func__, sn[0], sn[1], sn[8]);

    if ((sn[0] != SN0_VALUE) || (sn[1] != SN1_VALUE) || (sn[8] != SN8_VALUE)) {
        ret = -1;
        ERROR("%s sn value wrong!\n", __func__);
    }
...
static int atsha204a_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    atsha204a_sysdata_t *sha204_sysdata = NULL;
    int ret = 0;
    DEBUG("%s enter. I2C address: 0x%02x", __func__, client->addr);
    ...
    /* 检测芯片SN值确认是atsha204a芯片 */
    ret = check_chip_sn(client);
    if (ret) {
        ERROR("check_chip_sn error %d!", __func__, ret);
        goto ERR_1;
    }
...
{
    atsha204a_sysdata_t *sha204_sysdata = i2c_get_clientdata(client);
    if (IS_ERR_OR_NULL(sha204_sysdata))
        return 0;
...
static const struct of_device_id atsha204a_match_table[] = {
    { .compatible = "microchip,atsha204a", },
    { },
};
...
{
    DEBUG("%s Driver Version: %s", __func__, VERSION);
