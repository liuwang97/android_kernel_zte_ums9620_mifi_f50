		...
		const char *filepath, u16 curr_version)
{
	...
	memcpy(firmware->data, (char *)fw->data, firmware->size);
	...
	u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
	...
	/* Check firmware in file system when probe only when build to .ko */
		...
		if(firmware_from_file == NULL) {
			cts_info("%s firmware_request failed. try requeset %s\n", cts_fw_name, DEFAULT_UPDATE_FIRMWARE_NAME);
			firmware_from_file = cts_request_newer_firmware_from_fs(
		...
		const struct cts_firmware *firmware, bool to_flash)
{
	...
	cts_info("Update firmware ver: %04x device fw ver: %04x",
		FIRMWARE_VERSION(firmware), cts_dev->fwdata.version);
	if (!cts_dev->rtdata.force_update && (FIRMWARE_VERSION(firmware) <= cts_dev->fwdata.version)) {
		cts_info("Update firmware ver low device fw ver, don't need update");
	...
	ret = cts_sram_writel_retry(cts_dev, 0x00FFFC, firmware_info.firmware_sect_crc, 3, 1);
	if (ret != 0) {
	...
	ret = cts_sram_writel_retry(cts_dev, 0x00FFF8, firmware_info.firmware_sect_size, 3, 1);
	if (ret != 0) {
	...
	ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF0, 0xCC33555A, 3, 1);
	if (ret != 0) {
	...
	ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF8, firmware_info.firmware_sect_crc, 3, 1);
	if (ret != 0) {
	...
	ret = cts_hw_reg_writel_retry(cts_dev, 0x015FFC, firmware_info.firmware_sect_size, 3, 1);
	if (ret != 0) {
