...
#define cts_print_test_info(fmt, args...) do {\
	if (selftestdata->all_test_info != NULL) { \
		snprintf(selftestdata->all_test_info, TEST_ALL_INFO_LEN , fmt, ##args);\
		tpd_copy_to_tp_firmware_data(selftestdata->all_test_info);\
	} \
} while (0)

#define cts_print_result_info(fmt, args...) do {\
	if (selftestdata->all_test_info != NULL) { \
		snprintf(selftestdata->test_result_info, TEST_RESULT_INFO_LEN , fmt, ##args);\
		tpd_copy_to_tp_firmware_data(selftestdata->test_result_info);\
	} \
} while (0)
