...
int cts_resume_device(struct cts_device *cts_dev)
{
	...
	if (cts_charger_is_exist(cts_dev)) {
		cts_info("charger exist");
		cts_charger_plugin(cts_dev);
	} else {
		cts_info("charger not exist");
...
init_hwdata:
	...
	cts_info("Not request_firmware in %s, return", __func__);
#if 0
...
#ifdef CFG_CTS_CHARGER_DETECT
bool cts_charger_is_exist(struct cts_device *cts_dev)
{
