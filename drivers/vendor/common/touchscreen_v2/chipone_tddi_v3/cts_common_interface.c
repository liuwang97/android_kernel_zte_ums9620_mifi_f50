...
static int cts_init_tpinfo(struct ztp_device *cdev)
{
	...
	retval = cts_get_firmware_version(cts_dev, &cts_dev->fwdata.version);
	if (retval) {
		cts_err("Read firmware version failed %d", retval);
	}
...
static int cts_set_headset_state(struct ztp_device *cdev, int enable)
{
	...
	if (!cts_dev->rtdata.suspended) {
		if (cts_data->headset_mode) {
			cts_send_command(&cts_data->cts_dev, CTS_CMD_EARJACK_ATTACHED);
		} else {
			cts_send_command(&cts_data->cts_dev, CTS_CMD_EARJACK_DETACHED);
		}
	}
...
static int chipone_ts_resume(void * cts_dev)
{
	...
	return cts_driver_resume(cts_data);
}
...
static int cts_set_display_rotation(struct ztp_device *cdev, int mrotation)
{
	...
	case mRotatin_0:
		...
		ret = cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_CMD_MROTATION_FLAG, 0x00);
		break;
	case mRotatin_90:/*USB on right*/
		cts_info("mRotatin_90 01");
		ret = cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_CMD_MROTATION_FLAG, 0x01);
		break;
	case mRotatin_180:
	...
	case mRotatin_270:/*USB on left*/
		...
		ret = cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_CMD_MROTATION_FLAG, 0x02);
		break;
	default:
		break;
	}
...
static int cts_fw_upgrade(struct ztp_device *cdev, char *fw_name, int fwname_len)
{
	...
	cts_dev->rtdata.force_update = true;
...
int cts_copy_delta_raw_data(struct ztp_device *cdev, s16 *frame_data_words, int *len, enum tp_test_type  test_type)
{
	unsigned int col = 0;
	unsigned int row = 0;
	int retval = 0;
	...
	retval = cts_data_request(cdev, frame_data_words, test_type);
	if (retval < 0) {
...
static int  cts_testing_delta_raw_report(struct ztp_device *cdev, unsigned int num_of_reports)
{
	...
	for (idx = 0; idx < num_of_reports; idx++) {
		len += snprintf((char *)(cdev->tp_firmware->data + len), RT_DATA_LEN * 10 - len,
				"frame: %d, TX:%d  RX:%d\n", idx, row, col);
		retval = cts_copy_delta_raw_data(cdev, frame_data_words, &len, RAWDATA_TEST);
		if (retval < 0) {
			goto DATA_REQUEST_FAILED;
		}

		retval = cts_copy_delta_raw_data(cdev, frame_data_words, &len, DELTA_TEST);
		if (retval < 0) {
			goto DATA_REQUEST_FAILED;
		}
	}
