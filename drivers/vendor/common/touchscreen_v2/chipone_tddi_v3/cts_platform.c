		...
		const void *src, size_t len, int retry, int delay)
{
	...
	if (pdata->cts_dev->rtdata.program_mode) {
		...
		do {
			...
			if (ret) {
				cts_err("SPI write failed %d(%s)", ret, cts_strerror(ret));
				if (delay) {
					mdelay(delay);
				}
			} else {
		...
		int retry, int delay)
{
	...
	if (pdata->cts_dev->rtdata.program_mode) {
		...
		do {
			...
			if (ret) {
				cts_err("SPI read failed %d(%s)", ret, cts_strerror(ret));
				if (delay) {
					mdelay(delay);
				}
				continue;
...
static void cts_plat_handle_irq(struct cts_platform_data *pdata)
{
	...
	if (ret && !tpd_cdev->ignore_tp_irq) {
		cts_err("Device handle IRQ failed %d(%s)", ret, cts_strerror(ret));
	}
		...
		struct device_node *dev_node)
{
	...
	if (pdata->irq < 0) {
		cts_err("Parse irq failed %d(%s)", ret, cts_strerror(ret));
		return pdata->irq;
	}
		...
		struct device *device, enum cts_bus_type bus_type)
{
...
#endif /* CONFIG_CTS_SPI_HOST */
		ret = cts_plat_parse_dt(pdata, device->of_node);
		if (ret) {
			cts_err("Parse dt failed %d(%s)", ret, cts_strerror(ret));
			return ret;
		}
#endif /* CONFIG_CTS_OF */
	...
	if (IS_ERR(pdata->irq_thread)) {
		...
		cts_err("Run thread for IRQ failed %d(%s)", ret, cts_strerror(ret));
...
int cts_plat_request_resource(struct cts_platform_data *pdata)
{
	...
	if (ret) {
		cts_err("Request INT gpio (%d) failed %d(%s)",
			pdata->int_gpio, ret, cts_strerror(ret));
		goto err_out;
	}
	...
	if (ret) {
		cts_err("Request RST gpio (%d) failed %d(%s)",
			pdata->rst_gpio, ret, cts_strerror(ret));
		goto err_free_int;
	}
#endif /* CFG_CTS_HAS_RESET_PIN */
