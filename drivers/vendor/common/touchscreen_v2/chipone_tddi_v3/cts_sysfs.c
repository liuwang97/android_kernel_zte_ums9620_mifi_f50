...
#define MAX_ALL_INFO_LEN       (10 * 1024)
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (argc > 2) {
		...
		if(tp_alloc_tp_firmware_data(10 * MAX_ALL_INFO_LEN)) {
			cts_err(" alloc tp firmware data fail");
			goto err_free_data;
		}
		tpd_copy_to_tp_firmware_data((char *)data);
...
#define BASELINE_BUFFER_SIZE(cts_dev) \
	...
	baseline = kmalloc(BASELINE_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (baseline == NULL) {
		return scnprintf(buf, PAGE_SIZE,
		...
		struct device_attribute *attr, char *buf)
{
	...
	rawdata = kmalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (rawdata == NULL) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	rawdata_min = kmalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (rawdata_min == NULL) {
		cts_err("Alloc mem for rawdata min failed");
		ret = -ENOMEM;
		goto err_free_rawdata;
	}
	rawdata_max = kmalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (rawdata_max == NULL) {
		cts_err("Alloc mem for rawdata failed");
		ret = -ENOMEM;
...
int cts_sysfs_add_device(struct device *dev)
{
	...
	manualdiff_base = kzalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
	if (manualdiff_base == NULL) {
