...
#define CTS_DEV_NORMAL_MODE_I2CADDR         (0x58)
...
enum cts_device_fw_reg {
	...
	CTS_DEVICE_FW_REG_CMD_MROTATION_FLAG = 0x0801,
};
...
/* Firmware IDs, read from firmware register @ref CTS_DEV_FW_REG_CHIP_TYPE under normal mode */
enum cts_dev_fwid {
...
#ifdef CFG_CTS_CHARGER_DETECT
extern bool cts_charger_is_exist(struct cts_device *cts_dev);
extern int cts_charger_plugin(struct cts_device *cts_dev);
extern int cts_charger_plugout(struct cts_device *cts_dev);
extern int cts_deinit_charger_notifier(struct chipone_ts_data *cts_data);
#else /* CFG_CTS_CHARGER_DETECT */
static inline bool cts_charger_is_exist(struct cts_device *cts_dev) { return false; }
