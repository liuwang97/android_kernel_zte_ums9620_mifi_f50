...
*******************************************************************************/
static int ft8725_short_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < tdata->node.node_num; i++) {
        ...
        adcdata[i] = ((9 * tmp_adc + 4096) * 6355 / 100) / ((4096 - tmp_adc) * 9);
...
static int ft8725_open_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static int ft8725_cb_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int byte_num = 0;
...
static int ft8725_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int *rawdata = NULL;
...
static int ft8725_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static int start_test_ft8725(void)
{
    int ret = 0;
    ...
    if (true == test_item->short_test) {
        ret = ft8725_short_test(tdata, &temp_result);
    ...
    if (true == test_item->open_test) {
        ret = ft8725_open_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
    ...
    if (true == test_item->cb_test) {
        ret = ft8725_cb_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
    ...
    if (true == test_item->rawdata_test) {
        ret = ft8725_rawdata_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
    ...
    if (true == test_item->lcdnoise_test) {
        ret = ft8725_lcdnoise_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
...
struct test_funcs test_func_ft8725 = {
    .ctype = {0x29},
    .hwtype = IC_HW_INCELL,
    .key_num_total = 0,
    .start_test = start_test_ft8725,
};
