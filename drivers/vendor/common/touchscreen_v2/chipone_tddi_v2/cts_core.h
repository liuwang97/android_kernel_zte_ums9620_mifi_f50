...
/* struct cts_dev_ops {
	...
	int (*get_has_int_data)(const struct cts_device *cts_dev, bool *has_int_data);
	int (*get_int_data_method)(const struct cts_device *cts_dev, u8 *int_data_method);
	int (*set_int_data_method)(const struct cts_device *cts_dev, u8 int_data_method);
	int (*get_int_data_types)(const struct cts_device *cts_dev, u16 *int_data_types);
	int (*set_int_data_types)(const struct cts_device *cts_dev, u16 int_data_types);
	int (*calc_int_data_size)(struct cts_device *cts_dev);
	int (*polling_data)(const struct cts_device *cts_dev, u8 *buf, size_t size);
	int (*polling_test_data)(const struct cts_device *cts_dev, u8 *buf, size_t size);

	int (*top_get_rawdata)(struct cts_device *cts_dev, u8 *buf, size_t size);
	int (*top_get_manual_diff)(struct cts_device *cts_dev, u8 *buf, size_t size);
	int (*top_get_real_diff)(struct cts_device *cts_dev, u8 *buf, size_t size);
	int (*top_get_noise_diff)(struct cts_device *cts_dev, u8 *buf, size_t size);
	int (*top_get_basedata)(struct cts_device *cts_dev, u8 *buf, size_t size);
	int (*top_get_cnegdata)(struct cts_device *cts_dev, u8 *buf, size_t size);
	...
	int (*set_charger_state)(const struct cts_device *cts_dev, u8 enable);
	int (*get_charger_state)(const struct cts_device *cts_dev, u8 *isset);
	int (*set_earphone_state)(const struct cts_device *cts_dev, u8 enable);
...
#ifdef CFG_CTS_HEADSET_DETECT
extern int cts_earphone_plugin(struct cts_device *cts_dev);
extern int cts_earphone_plugout(struct cts_device *cts_dev);
#endif
