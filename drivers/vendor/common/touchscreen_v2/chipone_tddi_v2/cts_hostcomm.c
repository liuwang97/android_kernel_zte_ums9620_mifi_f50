...
int cts_hostcomm_get_swap_axes(const struct cts_device *cts_dev, bool *swap_axes)
{
	u8 val;
	int ret;
...
int cts_hostcomm_get_int_keep_time(const struct cts_device *cts_dev,
		u16 *int_keep_time)
{
	return cts_fw_reg_readw(cts_dev, CTS_DEVICE_FW_REG_INT_KEEP_TIME, int_keep_time);
}

int cts_hostcomm_get_esd_method(const struct cts_device *cts_dev, u8 *esd_method)
{
	return cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_ESD_PROTECTION, esd_method);
}
...
int cts_hostcomm_get_data_ready_flag(const struct cts_device *cts_dev, u8 *ready)
{
...
int cts_hostcomm_get_cneg(const struct cts_device *cts_dev, u8 *buf, size_t size)
{
	return cts_fw_reg_readsb_delay_idle(cts_dev,
		CTS_DEVICE_FW_REG_COMPENSATE_CAP, buf,
...
/*
	...
	return cts_plat_spi_read(cts_dev->pdata, CTS_DEV_NORMAL_MODE_SPIADDR,
...
{
	return cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_GET_WORK_MODE, workmode);
}
...
int cts_hostcomm_is_enabled_get_rawdata(const struct cts_device *cts_dev, u8 *enabled)
{
	return cts_fw_reg_readb(cts_dev, 0x12, enabled);
}

int cts_hostcomm_set_short_test_type(const struct cts_device *cts_dev, u8 short_type)
{
	return set_short_test_type(cts_dev, short_type);
}

int cts_hostcomm_is_openshort_enabled(const struct cts_device *cts_dev, u8 *enabled)
{
	int ret = -1;
...
int cts_hostcomm_set_openshort_enable(const struct cts_device *cts_dev, u8 enable)
{
	int ret = -1;
	cts_err("BUG! Should not be here!");
	return ret;
...
int cts_hostcomm_is_display_on(const struct cts_device *cts_dev, u8 *display_on)
{
...
int cts_hostcomm_top_get_rawdata(struct cts_device *cts_dev, u8 *buf, size_t size)
{
...
int cts_hostcomm_top_get_manual_diff(struct cts_device *cts_dev, u8 *buf, size_t size)
{
	//TODO:
	return 0;
}

int cts_hostcomm_top_get_real_diff(struct cts_device *cts_dev, u8 *buf, size_t size)
{
	//TODO:
	return 0;
}

int cts_hostcomm_top_get_noise_diff(struct cts_device *cts_dev, u8 *buf, size_t size)
{
	//TODO:
	return 0;
}

int cts_hostcomm_top_get_basedata(struct cts_device *cts_dev, u8 *buf, size_t size)
{
	/* TODO: */
	return 0;
}

int cts_hostcomm_top_get_cnegdata(struct cts_device *cts_dev, u8 *buf, size_t size)
{
...
struct cts_dev_ops hostcomm_ops = {
	...
	.get_module_id = NULL,
	.set_charger_state		= NULL,
	.set_earphone_state		= NULL,
};
