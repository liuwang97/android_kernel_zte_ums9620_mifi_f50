...
struct gcore_dev *gdev_intf = NULL;
...
struct proc_dir_entry *gcore_proc_entry = NULL;
...
{
	gcore_proc_entry = proc_create(GCORE_PROC_FILE, 0664, NULL, &gcore_app_fops);
	if (gcore_proc_entry == NULL) {
		GTP_ERROR("create proc entry gcore_app failed");
		return -EPERM;
	}

	gcore_mp_entry = proc_create(GCORE_MP_FILE, 0664, NULL, &gcore_selftest_fops);
