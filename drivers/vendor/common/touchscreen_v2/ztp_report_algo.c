...
static bool is_have_half_screen_area_point_down(u8 half_screen_area);
...
typedef struct point_fifo {
	tpd_point_info_t point_data[MAX_INSERT_POINTS + 2];
...
u16 dis_between_2_point(u16 x1, u16 y1 ,u16 x2, u16 y2)
{
	s32 disx = 0, disy = 0;

	disx = 	(s32)(x1 - x2);
	disy = 	(s32)(y1 - y2);
	return math_sqrt2(disx * disx + disy * disy);
}

static bool angle_suppression_area(u16 level, u16 x, u16 y)
{
	u16 r = 0;
	struct ztp_device *cdev = tpd_cdev;
	...
	if (cdev->display_rotation == mRotatin_90) {
		if ((dis_between_2_point(x, y, 0, 0) <= r) && is_have_half_screen_area_point_down(0))
			return true;
		if (( dis_between_2_point(x, y, 0, cdev->max_y) < r) && is_have_half_screen_area_point_down(1))
			return true;
	} else if (cdev->display_rotation == mRotatin_270) {
		if ((dis_between_2_point(x, y, cdev->max_x, 0) < r)  && is_have_half_screen_area_point_down(0))
			return true;
		if ((dis_between_2_point(x, y, cdev->max_x, cdev->max_y) <= r)  && is_have_half_screen_area_point_down(1))
			return true;
	}
	return false;
}
...
static bool point_is_in_limit_area(u16 x, u16 y)
{
	...
	if (angle_suppression_area(cdev->rotation_limit_level, x, y))
		return true;
...
static bool is_long_pess_clean_area(u16 x, u16 y)
{
	...
	if (cdev->display_rotation == mRotatin_90 || cdev->display_rotation == mRotatin_270) {
		if ((y < cdev->long_pess_suppression[2]) || (y > cdev->max_y - cdev->long_pess_suppression[3])) {
			return true;
		}
	} else {
		if ((x  < cdev->long_pess_suppression[0]) || (x > cdev->max_x - cdev->long_pess_suppression[1])) {
...
static bool is_report_point(u16 x, u16 y, u16 id, u8 touch_major, u8  pressure)
{
	...
	if (point->is_report_point) {
		if (is_need_clean_long_pess_area_down(point, x, y) == false)
			return true;
		if (is_long_pess_clean_area(x, y)) {
...
void get_interpolation_point(u16 id)
{
	int i = 0;
...
void tpd_touch_release(struct input_dev *input, u16 id)
{
	...
	if (point->finger_down) {
		...
		TPD_DMESG("tpd touch up id: %d, coord [%d:%d],down-up time:%d.\n",
