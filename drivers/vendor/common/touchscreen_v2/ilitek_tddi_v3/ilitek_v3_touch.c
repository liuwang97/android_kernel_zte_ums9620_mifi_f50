...
void ili_report_ap_mode(u8 *buf, int len)
{
	int i = 0;
	u32 xop = 0, yop = 0;
	static char finger_down[MAX_TOUCH_NUM] = {0};
	static struct tp_point_log point_log[MAX_TOUCH_NUM];
	...
	if (ilits->finger) {
		if (MT_B_TYPE) {
			for (i = 0; i < ilits->finger; i++) {
				...
				if (tpd_cdev->zte_tp_algo) {
					if (MT_PRESSURE)
						tpd_touch_press(ilits->input, touch_info[i].x, touch_info[i].y,
							touch_info[i].id, 0 ,touch_info[i].pressure);
					else
						tpd_touch_press(ilits->input, touch_info[i].x, touch_info[i].y,
							touch_info[i].id, 0 ,0);
				...
				input_report_key(ilits->input, BTN_TOOL_FINGER, 1);
#endif
				if (!finger_down[touch_info[i].id]) {
					finger_down[touch_info[i].id] = 1;
					ILI_INFO("touch down id: %d, coord [%d:%d]", touch_info[i].id,
						touch_info[i].x, touch_info[i].y);
				}
				point_log[touch_info[i].id].x = touch_info[i].x;
				point_log[touch_info[i].id].y = touch_info[i].y;
			}
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
				if (ilits->curt_touch[i] == 0 && ilits->prev_touch[i] == 1) {
#ifdef ILITEK_REPORT_BY_ZTE_ALGO
					if (tpd_cdev->zte_tp_algo)
						tpd_touch_release(ilits->input, i);
					else
						ili_touch_release(0, 0, i);
#else
					ili_touch_release(0, 0, i);
					...
					if (finger_down[i]) {
						finger_down[i] = 0;
						ILI_INFO("touch up id: %d, coord [%d:%d]", i, point_log[i].x, point_log[i].y);
					}
				}
...
void ili_report_proximity_mode(u8 *buf, int len)
{
	...
	if ((ilits->actual_tp_mode == P5_X_FW_AP_MODE) && (ilits->proxmity_face == true) && (ilits->power_status == true)) {
		ILI_DBG("ilits->proxmity_face not use\n");
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	case GESTURE_DOUBLECLICK:
		...
		ILI_DBG("Gesture Code=%d", GESTURE_DOUBLECLICK);
		if (tpd_cdev->tpd_report_uevent != NULL) {
			ILI_INFO("Double Click Uevent\n");
