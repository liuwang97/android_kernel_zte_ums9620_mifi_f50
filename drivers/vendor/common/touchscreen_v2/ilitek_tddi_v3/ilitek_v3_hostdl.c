...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	case REQUEST_FIRMWARE:
		if (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) < 0) {
			ILI_ERR("Request firmware failed, try to request default firmware\n");
			if (request_firmware(&fw, ilits->md_fw_default_path, ilits->dev) < 0) {
				ILI_ERR("Request default firmware failed after retry\n");
				ret = -1;
				goto out;
			}
		}
...
#endif
		break;
	case TP_FIRMWARE_DATA:
		if (cdev->tp_firmware != NULL && cdev->tp_firmware->data != NULL) {
			fsize = cdev->tp_firmware->size;
			ilits->tp_fw.size = 0;
			ilits->tp_fw.data = vmalloc(cdev->tp_firmware->size);
			if (ERR_ALLOC_MEM(ilits->tp_fw.data)) {
				ILI_ERR("Failed to allocate tp_fw by vmalloc, try again\n");
				ilits->tp_fw.data = vmalloc(cdev->tp_firmware->size);
				if (ERR_ALLOC_MEM(ilits->tp_fw.data)) {
			...
			/* Copy fw data got from request_firmware to global */
			ipio_memcpy((u8 *)ilits->tp_fw.data, cdev->tp_firmware->data , cdev->tp_firmware->size, fsize);
			ilits->tp_fw.size = cdev->tp_firmware->size;
		} else {
			ret = 0;
			goto out;
		}
