...
#endif
	if (action == do_report_after_reset) {
...
int semi_touch_guesture_switch(unsigned char en)
{
	...
	check_return_if_fail(ret, NULL);
	kernel_log_d("gusture command = %d\n", en);
...
int semi_touch_orientation_switch(unsigned char en)
{
	...
	kernel_log_d("rotation command = %d\n", en);

	enter_orientation_gate(st_dev.stc.ctp_run_status, en);

	return ret;
}

int semi_touch_report_rate_switch(unsigned short rate)
{
	int ret = 0, retry = 0;
	struct m_ctp_cmd_std_t cmd_send_tp;
	struct m_ctp_rsp_std_t ack_from_tp;
	...
	kernel_log_d("switch report rate: %d\n", rate);
...
int cmd_send_to_tp(struct m_ctp_cmd_std_t *ptr_cmd, struct m_ctp_rsp_std_t *ptr_rsp, int once_delay)
{
	...
	if(ret) {
		kernel_log_e("semi touch write bytes fail\n");
		return ret;
	}
...
int semi_touch_device_prob(void)
{
	...
	return (ret == SEMI_DRV_ERR_OK) ? SEMI_DRV_ERR_OK : -SEMI_DRV_ERR_HAL_IO;
}
...
{
	if (st_dev.asyn_work.work_queue[queue_type] == NULL) {
