...
#define CTP_ORIENTATION_GATE         (3 << 20)
#define ack_pointing_action(x)       ((x & 0x1ffff) == 0)
...
#define orientation_activity(x)      (((x) & CTP_ORIENTATION_GATE) >> 20)
#define enter_orientation_gate(x, g) do { ((x) |= (g << 20)); kernel_log_d("orientation status is %d...\n", (g)); } while (0)
...
typedef struct sm_touch_dev {
	...
	bool irq_enabled;
	unsigned short tp_self_test_result;
	bool is_double_tap;
	int is_single_tap;
	int is_single_aod;
	int is_single_fp;
