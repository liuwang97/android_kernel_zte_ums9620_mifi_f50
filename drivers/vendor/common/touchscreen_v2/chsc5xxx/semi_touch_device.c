...
extern void semi_tpd_register_fw_class(struct sm_touch_dev *st_dev);
...
static irqreturn_t semi_touch_irq_handler_imp(int irq, void *p)
{
	...
	unsigned char readbuffer[report_size];
#ifdef SEMI_REPORT_BY_ZTE_ALGO
	...
	if (!ack_pointing_action(st_dev.stc.ctp_run_status))
	...
	if ((int)read_and_report_touch_points(readbuffer, report_size) >= 0) {
		...
		if (readbuffer[0] == 0xFD) {
			/* firmware report heatbeat packet */
			return IRQ_RETVAL(IRQ_HANDLED);
		} else if (readbuffer[0] == 0xFC && semi_touch_proximity_report(readbuffer[1])) {
			return IRQ_RETVAL(IRQ_HANDLED);
		} else if (readbuffer[0] == 0xFE && semi_touch_gesture_report(readbuffer[1])) {
			return IRQ_RETVAL(IRQ_HANDLED);
		} else if(readbuffer[0] == 0xF1) {
			/* up & down */
			semi_touch_gesture_report((readbuffer[1] >> 4) & 0x0f);			
			return IRQ_RETVAL(IRQ_HANDLED);
		} else if(readbuffer[0] == 0xF0) {
			/* up & down */
			semi_touch_gesture_report((readbuffer[1] >> 4) & 0x0f);
		}
		...
		for (index = 0; index < SEMI_TOUCH_MAX_POINTS; index++) {
			/* EVENT_UP = 0x04 */
			pointed = (ppt->rp.event == 0x04) ? false : true;

			if (ppt->rp.id != 0x0f) {
				if (semi_touch_vkey_handled
				    (pointed, (unsigned int)(ppt->rp.x_h4 << 8) | ppt->rp.x_l8,
				     (unsigned int)(ppt->rp.y_h4 << 8) | ppt->rp.y_l8)) {

				} else {
					if (pointed) {
						input_report_key(st_dev.input, BTN_TOUCH, 1);
						input_report_abs(st_dev.input, ABS_MT_POSITION_X,
								 (unsigned int)(ppt->rp.x_h4 << 8) | ppt->rp.x_l8);
		...
		for (index = 0; index < SEMI_TOUCH_MAX_POINTS; index++) {
			...
			if (ppt->rp.id != 0x0f) {
				if (semi_touch_vkey_handled
					...
					if (cdev->zte_tp_algo) {
						if (pointed) {
							x = (u16)((ppt->rp.x_h4 << 8) | ppt->rp.x_l8);
							y = (u16)((ppt->rp.y_h4 << 8) | ppt->rp.y_l8);
							tpd_touch_press(st_dev.input, x, y, ppt->rp.id, ppt->rp.z, 0);
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(CONFIG_TOUCHSCREEN_POINT_SIMULAT_UF)
							uf_touch_report(x, y, 	ppt->rp.id);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
							one_key_report(true, x, y, ppt->rp.id);
#endif
						} else {
							tpd_touch_release(st_dev.input, ppt->rp.id);
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(CONFIG_TOUCHSCREEN_POINT_SIMULAT_UF)
							uf_touch_report(-1, -1, ppt->rp.id);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
							one_key_report(false, -1, -1, ppt->rp.id);
#endif
						}
					} else {
...
{
	if (semi_touch_wake_lock() == SEMI_DRV_ERR_OK) {
...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
		one_key_report(false, -1, -1, index);
#endif
	}
	input_report_key(st_dev.input, BTN_TOUCH, 0);
	input_sync(st_dev.input);
...
static int semi_touch_irq_init(struct sm_touch_dev *st_dev)
{
	...
	ret =
	    devm_request_irq(&client->dev, client->irq, semi_touch_irq_handler,
			     IRQF_TRIGGER_FALLING | IRQF_ONESHOT, client->name, st_dev);
#else
	ret =
	    devm_request_threaded_irq(&client->dev, client->irq, NULL, semi_touch_irq_handler_imp,
				      IRQF_TRIGGER_FALLING | IRQF_ONESHOT, client->name, st_dev);
#endif
	check_return_if_fail(ret, NULL);
...
int semi_touch_resolution_adaption(struct sm_touch_dev *st_dev)
{
	...
	for (index = 0; index < 3; index++) {
		...
		check_return_if_fail(ret, NULL);
		if (caculate_checksum_u16((unsigned short *)readbuffer, sizeof(readbuffer)) == 0) {
			break;
		}
	...
	for (index = 0; (index < st_dev->stc.vkey_num) && (index < MAX_VKEY_NUMBER); index++) {
		if (readbuffer[0x53] & 0x02) {
			...
			pix_y = (readbuffer[0x57 + index * 2] << 8) + readbuffer[0x56 + index * 2];
		} else {
			pix_y = (readbuffer[0x55] << 8) + readbuffer[0x54];
			pix_x = (readbuffer[0x57 + index * 2] << 8) + readbuffer[0x56 + index * 2];
		}
...
int semi_touch_init(struct i2c_client *client)
{
	...
	st_dev.iovdd_pin = semi_touch_get_iovdd_gpio();
	...
	semi_tpd_register_fw_class(&st_dev);
