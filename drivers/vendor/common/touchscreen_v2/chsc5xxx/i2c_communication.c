...
int i2c_write_bytes(struct hal_io_packet *ppacket)
{
	...
	for (retry = 0; retry < HAL_MAX_TRY; retry++) {
		...
		if (ppacket->io_length + sizeof(int) == ret)
			break;
	}
	client->addr = client->addr & I2C_MASK_FLAG & (~I2C_DMA_FLAG);
...
{
	if (dma_buff_virtual_addr == NULL) {
		st_dev.input->dev.coherent_dma_mask = DMA_BIT_MASK(32);
		dma_buff_virtual_addr =
		    dma_alloc_coherent(&st_dev.input->dev, DMA_BUFFER_LENGTH, &dma_buff_physical_addr, GFP_KERNEL);
...
int i2c_write_bytes(struct hal_io_packet *ppacket)
{
	...
	for (retry = 0; retry < HAL_MAX_TRY; retry++) {
		...
		if (ret == 1) {
			...
			kernel_log_d("i2c write error retry: %d!!",retry + 1);
...
int i2c_read_bytes(struct hal_io_packet *ppacket)
{
	...
	for (retry = 0; retry < HAL_MAX_TRY; retry++) {
		...
		if (ret == 2) {
			...
			kernel_log_d("i2c read error retry: %d!!",retry + 1);
			usleep_range(10000, 11000);
		}
	}
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
