...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		if (iovdd_gpio > 0) {
			ts_info("iovdd_gpio output high");
...
/*#define PLAY_GAME_ADDR 0x10180*/
int zte_enter_play_game(struct goodix_ts_core *cd, int enable)
{
	struct goodix_ts_cmd cmd;

	/*cd->ic_info.misc.cmd_addr = PLAY_GAME_ADDR;*/
	if (cd->bus->ic_type == IC_TYPE_BERLIN_A) {
		cmd.cmd = 0x1A;
		cmd.len = 0x6;
		switch (enable) {
			case tp_freq_180Hz:
				ts_info("%s 180Hz", __func__);
				cmd.data[0] = 0x00;/*180Hz*/
				cmd.data[1] = 0x05;
				cmd.data[2] = 0x25;
				break;
			case tp_freq_240Hz:
...
int zte_tp_set_report_rate(struct goodix_ts_core *cd, int mode)
{
	struct goodix_ts_cmd cmd;
	static int current_report_mode = tp_freq_180Hz;
	...
	if (cd->bus->ic_type == IC_TYPE_BERLIN_A) {
		...
		if (current_report_mode == tp_freq_360Hz && mode != tp_freq_360Hz) {
		...
		switch (mode) {
			...
			case tp_freq_180Hz:
				ts_info("%s 180Hz", __func__);
				current_report_mode = tp_freq_180Hz;
				cmd.data[0] = 0x01;/*180Hz*/
				cmd.data[1] = 0xA3;
				cmd.data[2] = 0x0;
				break;
...
int zte_leave_play_game(struct goodix_ts_core *cd)
{
	struct goodix_ts_cmd cmd;
