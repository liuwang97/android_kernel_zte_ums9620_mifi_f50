...
extern void goodix_ts_esd_off(struct goodix_ts_core *cd);
...
int gtp_get_vendor_and_firmware(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(goodix_vendor_l); i++) {
		if (strnstr(lcd_name, goodix_vendor_l[i].vendor_name, strlen(lcd_name))) {
...
static void goodix_work_charger_detect_work(struct work_struct *work)
{
	...
	if (core_data->init_stage < CORE_INIT_STAGE2) {
		ts_err("%s: error, TP is not initialized!", __func__);
		return;
	}
...
#ifdef GOODIX_USB_DETECT_GLOBAL
	cancel_delayed_work_sync(&core_data->charger_work);
#endif
#ifdef PEN_LOW_BATT_CHECK
	cancel_delayed_work_sync(&core_data->pen_low_batt_check_work);
#endif
