...
static int parse_dt(struct device *dev, struct ovt_tcm_board_data *bdata)
{
	...
	ovt_info(INFO_LOG, "%s:bdata->irq_gpio=%d\n", __func__, bdata->irq_gpio);
	...
	ovt_info(INFO_LOG, "%s:bdata->irq_on_state=%d\n", __func__, bdata->irq_on_state);
	...
	ovt_info(INFO_LOG, "%s:bdata->power_gpio=%d\n", __func__, bdata->power_gpio);
	...
	ovt_info(INFO_LOG, "%s:bdata->power_on_state=%d\n", __func__, bdata->power_on_state);
	...
	ovt_info(INFO_LOG, "%s:bdata->power_delay_ms=%d\n", __func__, bdata->power_delay_ms);
	...
	ovt_info(INFO_LOG, "%s:bdata->reset_gpio=%d\n", __func__, bdata->reset_gpio);
	...
	ovt_info(INFO_LOG, "%s:bdata->reset_active_ms=%d\n", __func__, bdata->reset_active_ms);
	...
	ovt_info(INFO_LOG, "%s:bdata->reset_delay_ms=%d\n", __func__, bdata->reset_delay_ms);
	...
	ovt_info(INFO_LOG, "%s:bdata->tpio_reset_gpio=%d\n", __func__, bdata->tpio_reset_gpio);
	...
	ovt_info(INFO_LOG, "%s:bdata->byte_delay_us=%d\n", __func__, bdata->byte_delay_us);
	...
	ovt_info(INFO_LOG, "%s:bdata->block_delay_us=%d\n", __func__, bdata->block_delay_us);
	...
	ovt_info(INFO_LOG, "%s:bdata->spi_mode=%d\n", __func__, bdata->spi_mode);
	...
	ovt_info(INFO_LOG, "%s:bdata->ubl_max_freq=%d\n", __func__, bdata->ubl_max_freq);
	...
	ovt_info(INFO_LOG, "%s:bdata->ubl_byte_delay_us=%d\n", __func__, bdata->ubl_byte_delay_us);

	/*zte_add*/
	prop = of_find_property(np, "omnivision,panel-max-x", NULL);
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "omnivision,panel-max-x",
				&value);
		if (retval < 0) {
			ovt_info(ERR_LOG, "Unable to read omnivision,panel-max-x property\n");
			return retval;
		} else {
			bdata->panel_max_x = value;
		}
	} else {
		bdata->panel_max_x = 0;
	}

	prop = of_find_property(np, "omnivision,panel-max-y", NULL);
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "omnivision,panel-max-y",
				&value);
		if (retval < 0) {
			ovt_info(ERR_LOG, "Unable to read omnivision,panel-max-y property\n");
			return retval;
		} else {
			bdata->panel_max_y = value;
		}
	} else {
		bdata->panel_max_y = 0;
	}

	ovt_info(INFO_LOG, "%s:PANEL_MAX_X:%d\n", __func__, bdata->panel_max_x);
	ovt_info(INFO_LOG, "%s:PANEL_MAX_Y:%d\n", __func__, bdata->panel_max_y);
...
static int ovt_tcm_spi_probe(struct spi_device *spi)
{
	...
	ovt_info(DEBUG_LOG, "%s:spi_mode=%d\n", __func__, hw_if.bdata->spi_mode);
