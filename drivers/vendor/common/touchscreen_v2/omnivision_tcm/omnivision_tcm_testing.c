...
\
	...
	for (i = 0; i < rows; i++) { \
		for (j = 0; j < cols; j++) { \
			data_value = testing_hcd->report.buf[(i * rows + j) * 2] + (testing_hcd->report.buf[(i * rows + j) * 2 + 1] * 256); \
...
int testing_do_testing(void);

int testing_pt11_open_detection(void);

int testing_raw_data(void);

int testing_delta_data(void);
...
static int ovt_tcm_get_thr_from_csvfile(void)
{
	...
	if (ret) {
		ovt_info(INFO_LOG, "ovt tcm csv parser: %s: Failed get %s \n", __func__, CSV_RAW_DATA_MIN_ARRAY);
		return ret;
	}

	ret = ovt_tcm_parse_csvfile(tcm_hcd, CSV_RAW_DATA_MAX_ARRAY,
	...
	ret = ovt_tcm_parse_csvfile(tcm_hcd, CSV_OPEN_SHORT_MIN_ARRAY,
	...
	ret = ovt_tcm_parse_csvfile(tcm_hcd, CSV_OPEN_SHORT_MAX_ARRAY,
	...
	ret = ovt_tcm_parse_csvfile(tcm_hcd, CSV_LCD_NOISE_ARRAY,
	...
	ovt_info(INFO_LOG, "ovt tcm csv parser: %s: success \n", __func__);
	return 0;
}
...
static int testing_do_test_item(enum test_code test_item, int limit_rows, int limit_cols, int* limit_data_low, int *limit_data_high, struct file *fp, char *output_str, int str_size)
{
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			...
			if (output_str) {
				snprintf(output_str + strlen(output_str), str_size - strlen(output_str),  "%5d,", data);
			}
			...
			if (limit_data_low) {
				if (data < limit_low) {
					testing_hcd->result = false;
					ovt_info(ERR_LOG,
						"fail at (%2d, %2d) data = %5d, limit_low = %4d\n",
			...
			if (limit_data_high) {
				if (data > limit_high) {
					testing_hcd->result = false;
					ovt_info(ERR_LOG,
						"fail at (%2d, %2d) data = %5d, limit_high = %4d\n",
...
int testing_do_testing(void)
{
	...
	if (retval < 0) {
		ovt_info(INFO_LOG, "ovt_tcm_get_thr_from_csvfile error, return\n");
		snprintf(g_testing_output_buf + strlen(g_testing_output_buf), OUTPUT_TO_CSV_STRING_LEN - strlen(g_testing_output_buf), 
	...
	retval = testing_do_test_item(TEST_PT7_DYNAMIC_RANGE, TX_NUM_MAX, RX_NUM_MAX, testing_hcd->testing_csv_threshold.raw_data_min_limits, 
		testing_hcd->testing_csv_threshold.raw_data_max_limits, NULL, g_testing_output_buf, OUTPUT_TO_CSV_STRING_LEN);
	if (retval < 0) {
		error_count++;
		ovt_tcm_tptest_result = ovt_tcm_tptest_result | OVT_TCM_TEST_BEYOND_MAX_LIMIT | OVT_TCM_TEST_BEYOND_MIN_LIMIT;
	}

	retval = testing_do_test_item(TEST_PT10_DELTA_NOISE, TX_NUM_MAX, RX_NUM_MAX, NULL, 
		testing_hcd->testing_csv_threshold.lcd_noise_max_limits, NULL, g_testing_output_buf, OUTPUT_TO_CSV_STRING_LEN);
	if (retval < 0) {
		error_count++;
		ovt_tcm_tptest_result = ovt_tcm_tptest_result | OVT_TCM_TEST_BEYOND_MAX_LIMIT | OVT_TCM_TEST_BEYOND_MIN_LIMIT;
	}
	retval = testing_do_test_item(TEST_PT11_OPEN_DETECTION, TX_NUM_MAX, RX_NUM_MAX, testing_hcd->testing_csv_threshold.open_short_min_limits, 
		testing_hcd->testing_csv_threshold.open_short_max_limits, NULL, g_testing_output_buf, OUTPUT_TO_CSV_STRING_LEN);
	if (retval < 0) {
		error_count++;
		ovt_tcm_tptest_result = ovt_tcm_tptest_result | OVT_TCM_TEST_GT_OPEN;
	}
	tpd_copy_to_tp_firmware_data(g_testing_output_buf);	
#endif
sys_err:
	if (error_count) {
		return -1;
...
int testing_raw_data(void)
{
	...
	if (retval >= 0) {
		ovt_info(INFO_LOG,"success to collect raw data\n");
	} else {
		ovt_info(ERR_LOG,"fail to collect raw data\n");
	}
	return retval;
}
