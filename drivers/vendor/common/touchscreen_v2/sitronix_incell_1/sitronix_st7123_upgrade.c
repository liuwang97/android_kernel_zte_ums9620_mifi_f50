...
{
	int ret = 0;
	bool oldIrqStatus = false;
	st_u8 cmd[6];
	gts->upgrade_doing = true;
	oldIrqStatus = sitronix_ts_get_irq_status();
	st_irq_off();
	...
	if (oldIrqStatus) {
		stmsg("sitronix_spi_hdl_fw restore irq on.\n");
		st_irq_on();
	} else {
		stmsg("sitronix_spi_hdl_fw restore irq off.\n");
		st_irq_off();
	}
...
int sitronix_do_upgrade(void)
{
	...
	if (adb_upgrade_firmware && adb_upgrade_firmware->data) {
		dump_buf = (unsigned char *) adb_upgrade_firmware->data;
	} else {
		while(retry < 3) {
		...
		if ( ret )
			...
			stmsg("request_firmware OK! firmware size:%d\n", fw->size);
