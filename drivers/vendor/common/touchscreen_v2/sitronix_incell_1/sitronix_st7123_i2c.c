...
static int sitronix_ts_i2c_parse_dt(struct device *dev, struct sitronix_ts_host_interface *host_if)
{
	...
	if (host_if->irq_gpio < 0) {
		...
		if (gpio_request(host_if->irq_gpio, "stx_irq_gpio")) {
			sterr("[GPIO]irq gpio request failed");
			return -EPERM;
		}
		gpio_direction_input(host_if->irq_gpio);
	...
	if (host_if->rst_gpio < 0) {
		...
		if (gpio_request(host_if->rst_gpio, "stx_rst_gpio")) {
			sterr("[GPIO]reset gpio request failed");
			gpio_free(host_if->irq_gpio);
			return -EPERM;
		}
		gpio_direction_output(host_if->rst_gpio, 0);
...
static int sitronix_ts_i2c_write(uint16_t addr, uint8_t *data, uint16_t length, void *if_data)
{
	...
	if(length + 2 > I2C_BUFFER_SIZE) {
		kfree(i2c_buf);
		for(retry = 0; retry < 3; retry++){
			i2c_buf = kzalloc(length + 2, GFP_KERNEL);
			msg->buf = i2c_buf;
			if(i2c_buf != NULL){
				break;
			}
		...
		if(retry >= 3){
			sterr("%s: Failed to kzalloc i2c_buf\n", __func__);
			return -ENOMEM;
		}
	}
...
static int sitronix_ts_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for(retry = 0; retry < 3; retry++){
		i2c_buf = kzalloc(I2C_BUFFER_SIZE, GFP_KERNEL);
		if(i2c_buf != NULL){
			break;
		}
	}
