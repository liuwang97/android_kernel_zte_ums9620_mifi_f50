...
static int sqc_hal_pd_set_apdo_cap(int type, int mV, int mA)
{
	struct sqc_pd_proto_ops *pd_chg_ops = pd_hal_data.pd_chg_ops;
	int ret = 0;

	if (pd_chg_ops != NULL && pd_chg_ops->set_apdo_cap != NULL)
		ret = pd_chg_ops->set_apdo_cap(type, mV, mA);

	return ret;
}

static int sqc_hal_pd_get_apdo_cap(struct sqc_pd_adapter_cap *pdos)
{
	struct sqc_pd_proto_ops *pd_chg_ops = pd_hal_data.pd_chg_ops;
	int ret = 0;

	if (pd_chg_ops != NULL && pd_chg_ops->get_apdo_cap != NULL)
		ret = pd_chg_ops->get_apdo_cap(pdos);
			   ...
			   size_t count, loff_t *f_pos)
{
	...
	struct sqc_pd_adapter_cap pdos;
	...
	case SQC_IFS_PTL_PD_APDO:
		memset(&pdos, 0, sizeof(struct sqc_pd_adapter_cap));
		retval = sqc_hal_pd_get_apdo_cap(&pdos);
		read_cnt = sizeof(pdos);
		break;
	default:
		retval = -ENOMEM;
	...
	if (retval >= 0) {
		if (*f_pos != SQC_IFS_PTL_PD_APDO) {
			if (copy_to_user(buf, temp_val, (count < read_cnt) ? count : read_cnt)) {
				pr_err("%s copy_to_user failed!\n", __func__);
				retval = -EFAULT;
			}
		} else {
			if (copy_to_user(buf, &pdos, (count < read_cnt) ? count : read_cnt)) {
				pr_err("%s copy_to_user failed!\n", __func__);
				retval = -EFAULT;
			}
		}
