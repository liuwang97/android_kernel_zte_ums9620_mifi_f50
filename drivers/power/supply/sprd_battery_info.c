...
*/
static int sprd_battery_get_batt_id_kohm(struct power_supply *psy, struct sprd_battery_info *data)
{
	int vol, batt_id_kohm, ret;

	if (IS_ERR_OR_NULL(data->batt_id_cha)) {
		dev_warn(&psy->dev, "%s batt id channel is not defined!\n", __func__);
		return -ENODEV;
	}

	ret = iio_read_channel_processed(data->batt_id_cha, &vol);
	if (ret < 0) {
		dev_warn(&psy->dev, "%s batt id channel read failed!\n", __func__);
		return ret;
	}
...
int sprd_battery_parse_battery_id(struct power_supply *psy, struct sprd_battery_info *data)
{
	int len = 0, i = 0;
	...
	if (of_get_property(battery_np, "batt-id-list", &len)) {
		len /= sizeof(u32);
		data->batt_id_list =
			kzalloc(sizeof(int) * len, GFP_KERNEL);
		if (!data->batt_id_list)
			return -ENOMEM;
		dev_info(&psy->dev, "batt-id-list num: %d\n", len);
		for (i = 0; i < len; i++) {
			if (of_property_read_u32_index(battery_np, "batt-id-list",
				i, &data->batt_id_list[i])) {
				dev_err(&psy->dev, "parse bat-id-list err\n");
			}
			dev_info(&psy->dev, "batt_id_list[%d]=%d K\n", i, data->batt_id_list[i]);
		}

		for (i = 0; i < len; i++) {
			/*  To solve the problem that the ID resistance is too small to match
				If the sampling resistance is less than 10K, judge the absolute is less than 5K*/
			if ((data->batt_id_kohm < 10)
				&& (abs(data->batt_id_kohm - data->batt_id_list[i]) < 5)) {
				data->batt_id_index = i;
				dev_info(&psy->dev, "batt id: %dk, near the batt_id_list[%d]: %dk\n",
			...
			*/
			if (data->batt_id_kohm >= data->batt_id_list[i] * 80/100
				&& data->batt_id_kohm <= data->batt_id_list[i] * 120/100) {
				data->batt_id_index = i;
				dev_info(&psy->dev, "batt id: %dk, near the batt_id_list[%d]: %dk\n",
					data->batt_id_kohm, data->batt_id_index, data->batt_id_list[i]);
				break;
		...
		if (i >= len)
			dev_info(&psy->dev, "batt id: %dk, not found the best index, use index 0\n",
							data->batt_id_kohm);
	} else {
		dev_info(&psy->dev, "batt id: %dk, no batt id list, use index 0\n", data->batt_id_kohm);
		return -ENOMEM;
	}
					     ...
					     struct power_supply *psy)
{
	...
	for (index = 0; index < fcc_table_len; index++) {
		...
		for (i = 0; i < ocv_table_len; i++) {
			...
			dev_dbg(&psy->dev, "index %d: [%d]: ocv %d, cap: %d\n",
						index, i, ocv_table[i].ocv, ocv_table[i].capacity);
...
int sprd_battery_get_battery_info(struct power_supply *psy, struct sprd_battery_info *info)
{
	...
	info->batt_id_index = 0;

	info->fullbatt_advance_level = FULLBATT_ADVANCE_LEVEL;
	...
	sprd_battery_parse_battery_id(psy, info);

	dev_warn(&psy->dev, "%s info->batt_id_index=%d\n", __func__, info->batt_id_index);

	battery_np = of_parse_phandle(psy->of_node, "monitored-battery", info->batt_id_index);
	if (!battery_np) {
	...
	err = of_property_read_u32(battery_np, "charge-full-microamp-hours",
			     &info->charge_full_uah);
	if (err) {
		info->charge_full_uah = info->charge_full_design_uah;
	}
	...
	of_property_read_u32(battery_np, "fullbatt-advance-level", &info->fullbatt_advance_level);
...
int sprd_battery_get_battery_info_sqc(struct power_supply *psy, struct sprd_battery_info *info, int battery_id)
{
	struct device_node *battery_np;
	const char *value;
	int err, index;
	...
	info->cur.wl_epp_cur = -EINVAL;
	info->cur.wl_epp_limit = -EINVAL;
	if (!strcmp(psy->desc->name, "sc27xx-fgu")) {
		info->batt_id_index = 0;
	} else {
		info->batt_id_index = battery_id;
	}
...
EXPORT_SYMBOL_GPL(sprd_battery_get_battery_info_sqc);
#endif
