...
static void fullbatt_vchk(struct work_struct *work)
{
	...
	if (charger_policy_get_status() == true) {
		dev_info(cm->dev, "%s: ignore fucntion when policy is enabled\n", __func__);
		return;
	}
...
static int cm_feed_watchdog(struct charger_manager *cm)
{
	...
	psy = zte_power_supply_get_by_name("zte_bq2560x_charger");
	if (!psy) {
		dev_err(cm->dev, "Cannot find power supply \"zte_bq2560x_charger\", not feed watchdog!\n");
		return 0;
	}

	val.intval = cm->desc->wdt_interval;
	err = zte_power_supply_set_property(psy,
...
static void misc_event_handler(struct charger_manager *cm, enum cm_event_types type)
{
	...
	if (is_ext_pwr_online(cm)) {
		dev_info(cm->dev, "%s,line:%d,usb changed online\n", __func__, __LINE__);
		...
		dev_info(cm->dev, "%s,line:%d,usb changed offline\n", __func__, __LINE__);
		if (cm->desc->xts_limit_cur)
			cm_power_path_enable(cm, CM_POWER_PATH_ENABLE_CMD);
		try_wireless_charger_enable(cm, false);
		cm_enable_fixed_fchg_handshake(cm, false);
...
#ifndef CONFIG_VENDOR_SQC_CHARGER
...
int charger_manager_get_ship_mode(struct charger_manager *cm)
{
	...
	if (cm && cm->desc && cm->desc->zte_psy_hardware) {
		...
		dev_err(cm->dev, "no zte_psy_hardware node!\n");
		return -1;
	}
...
int charger_manager_set_ship_mode(struct charger_manager *cm, int enable)
{
	...
	if (cm && cm->desc && cm->desc->zte_psy_hardware) {
		psy = zte_power_supply_get_by_name(cm->desc->zte_psy_hardware);
		if (!psy) {
			dev_err(cm->dev, "get zte_psy_hardware psy failed!!\n");
			return -ENODEV;
		}
		...
		rc = zte_power_supply_set_property(psy,
				POWER_SUPPLY_PROP_SET_SHIP_MODE, &val);
		if (rc < 0) {
			dev_err(cm->dev, "Failed to set zte_psy_hardware shipmode failed\n");
			zte_power_supply_put(psy);
			return -ENODEV;
		}
		...
		if (cm->desc->zte_psy_hardware2) {
			psy = zte_power_supply_get_by_name(cm->desc->zte_psy_hardware2);
			if (!psy) {
				dev_err(cm->dev, "get zte_psy_hardware2 psy failed!!\n");
				return -ENODEV;
			}
			...
			rc = zte_power_supply_set_property(psy,
					POWER_SUPPLY_PROP_SET_SHIP_MODE, &val);
			if (rc < 0) {
				dev_err(cm->dev, "Failed to set zte_psy_hardware2 shipmode failed\n");
				zte_power_supply_put(psy);
				return -ENODEV;
			}
			       ...
			       struct charger_desc *desc, struct device *dev)
{
	...
	dev_warn(dev, "%s jeita_size is: %d\n", desc->jeita_tab_size);
...
static int cm_get_bat_info(struct charger_manager *cm)
{
	...
	int ret;
#ifdef CONFIG_VENDOR_SQC_CHARGER
	ret = sprd_battery_get_battery_info_sqc(cm->charger_psy, &info, cm->battery_id);
#else
	...
	dev_info(cm->dev, "SPRD_BATTERY_INFO: internal_resist= %d, us= %d, constant_charge_voltage_max_uv= %d, fchg_ocv_threshold= %d, cp_target_vbat= %d, cp_max_ibat= %d, cp_target_ibat= %d, cp_max_ibus= %d,\n"
			"sdp_limit= %d, sdp_cur= %d, dcp_limit= %d, dcp_cur= %d, cdp_limit= %d, cdp_cur= %d unknown_limit= %d, unknown_cur= %d, fchg_limit= %d, fchg_cur= %d, flash_limit= %d, flash_cur= %d, wl_bpp_limit= %d,\n"
			"wl_bpp_cur= %d, wl_epp_limit= %d, wl_epp_cur= %d, fullbatt_uV= %d, fullbatt_uA= %d, cm->desc->first_fullbatt_uA= %d, us_upper_limit= %d, rc= %d, cp_upper_limit_offset= %d,fullbatt_advance_level=%d, cm->battery_id=%d,\n"
			"temp_min=%d\n",
		 ...
		 cm->desc->ir_comp.cp_upper_limit_offset, cm->desc->fullbatt_advance_level, cm->battery_id, cm->desc->temp_min);
	...
	case POWER_SUPPLY_STATUS_FULL:
		...
		if (cm->health == POWER_SUPPLY_HEALTH_WARM) {
			if ((batt_ocV < (cm->desc->fullbatt_warm_uV - cm->desc->fullbatt_vchkdrop_uV))
					&& (batt_uA < 0)) {
				cm->desc->force_set_full = false;
				dev_info(cm->dev, "force_set_full %d, health %d, batt_ocv %d, batt_uA %d\n",
					cm->desc->force_set_full, cm->health, batt_ocV, batt_uA);
			}
		} else if ((batt_ocV < (cm->desc->fullbatt_uV - cm->desc->fullbatt_vchkdrop_uV - 10000))
		    && (batt_uA < 0)) {
			cm->desc->force_set_full = false;
			dev_info(cm->dev, "force_set_full %d, batt_ocv %d, batt_uA %d.\n",
				cm->desc->force_set_full, batt_ocV, batt_uA);
		}
...
static int charger_manager_probe(struct platform_device *pdev)
{
	...
	struct charger_desc *desc = NULL;
	struct charger_manager *cm;
	int ret, i = 0;
	union power_supply_propval val;
	struct power_supply *fuel_gauge;
	struct zte_power_supply *zte_fuel_gauge;
	...
	psy_hardware_chg = power_supply_get_by_name(psy_hardware_name);
	if (!psy_hardware_chg) {
		dev_err(&pdev->dev, "get %s psy failed, defer probe\n", psy_hardware_name);
		return -EPROBE_DEFER;
	}
	power_supply_put(psy_hardware_chg);
	...
	if (cm->desc->zte_psy_fuel_gauge) {
		zte_fuel_gauge = zte_power_supply_get_by_name(cm->desc->zte_psy_fuel_gauge);
		if (!zte_fuel_gauge) {
			dev_err(cm->dev, "failed to get psy cm->desc->zte_psy_fuel_gauge\n");
			return -EPROBE_DEFER;
		}
		ret = zte_power_supply_get_property(zte_fuel_gauge,
				POWER_SUPPLY_PROP_BATTERY_ID,
				&val);
		cm->battery_id = val.intval;
		dev_info(cm->dev, "cm->battery_id=%d\n", cm->battery_id);
		zte_power_supply_put(zte_fuel_gauge);
	}
