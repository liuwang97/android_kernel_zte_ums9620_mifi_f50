...
static enum power_supply_usb_type sprd_vchg_get_bc1p2_type(struct sprd_vchg_info *info)
{
	...
	/*add by zte, force report float type when pd hard reset*/
	if (info->typec_online && (usb_type == POWER_SUPPLY_USB_TYPE_UNKNOWN)) {
		dev_err(info->dev, "%s: typec_online force report float\n", SPRD_VCHG_TAG);
		usb_type = NON_DCP_TYPE;
	}
