...
#define SC27XX_FGU_LOW_VBAT_UUSOC_STEP	5
...
#define SC27XX_FGU_SHUTDOWN_FOURTEEN_DAYS		(1209600)
/* 3 days (3*24*60*60) = (259200) */
#define SC27XX_FGU_SHUTDOWN_THREE_DAYS		(259200)
/* 300mV */
#define SC27XX_FGU_OCV_VALID_THRESHOLD		(300*1000)
...
static void sc27xx_fgu_capacity_loss_by_temperature(struct sc27xx_fgu_data *data, int *cap)
{
	...
	if (data->cap_table_len > 0) {
		...
		*cap = DIV_ROUND_CLOSEST((*cap + temp_cap - 1000) * 1000, temp_cap);
		...
		dev_info(data->dev, "%s temp_cap=%d,cap=%d\n", __func__, temp_cap, *cap);
...
static void sc27xx_fgu_dump_battery_info(struct sc27xx_fgu_data *data, char *str)
{
	...
	dev_info(data->dev, "%s, ocv_table_len = %d, temp_table_len = %d, rabat_table_len = %d, "
		 ...
		 "internal_resist = %d, min_volt_uv = %d, batt_id_index = %d\n",
		 ...
		 data->internal_resist, data->min_volt_uv, data->batt_id_index);
	...
	if (data->cap_temp_table) {
		for (i = 0; i < data->cap_table_len; i++)
			dev_info(data->dev, "cap_temp_table[%d].temp = %d, cap_temp_table[%d].cap = %d\n",
				 i, data->cap_temp_table[i].temp, i, data->cap_temp_table[i].cap);
	}
}
...
static int sc27xx_fgu_save_normal_temperature_cap(struct sc27xx_fgu_data *data, int cap)
{
	...
	dev_info(data->dev, "%s: value = %d,cap=%d\n", __func__, value, cap);
...
static int sc27xx_fgu_read_normal_temperature_cap(struct sc27xx_fgu_data *data, int *cap)
{
	...
	dev_info(data->dev, "%s,line=%d: value = %d, cap=%d\n", __func__, __LINE__, value, *cap);

	if (ret) {
		dev_err(data->dev, "%s:error!\n", __func__);
		return ret;
	}
					    ...
					    int pocv_cap, int pocv_uv, int *cap)
{
	...
	/* Check whether the voltage difference between pocv and ocv exceeds 300mV */
	if (abs(pocv_uv - ocv_mv * 1000) > SC27XX_FGU_OCV_VALID_THRESHOLD) {
		dev_err(data->dev, "pocv_uv = %d, ocv_uv = %d, delta = %d\n",
			pocv_uv, ocv_mv * 1000, abs(pocv_uv - ocv_mv * 1000));
		return;
	}
...
/*
 *  If the poweroff time exceeds 14 days, set the is_first_poweron flag as true.
 *  Fix the issue that the ui_cap is inconsistent with the actual cap level after long-time poweroff.
 */
static bool sc27xx_fgu_boot_after_long_poweroff(struct sc27xx_fgu_data *data, s64 *poweroff_time)
{
	s64 cur_time = 0, shutdown_time = 0;
	int ret = 0;

	*poweroff_time = 0;
	if (data->shutdown_rtc_time == 0 || data->shutdown_rtc_time == -1 ||
	    data->shutdown_rtc_time < SC27XX_FGU_MISCDATA_RTC_TIME) {
		dev_err(data->dev, "%s : shutdown_rtc_time = %lld not meet\n",
			__func__, data->shutdown_rtc_time);
		return false;
	}

	ret = sc27xx_fgu_get_rtc_time(data, &cur_time);
	if (ret) {
		dev_err(data->dev, "%s : get current rtc_time failed, ret = %d\n", __func__, ret);
		return false;
	}

	if (cur_time < SC27XX_FGU_MISCDATA_RTC_TIME) {
		dev_info(data->dev, "%s : current rtc time = %lld less than %d\n",
			__func__, cur_time, SC27XX_FGU_MISCDATA_RTC_TIME);
		return false;
	}

	shutdown_time = cur_time - data->shutdown_rtc_time;
	*poweroff_time = shutdown_time;

	dev_info(data->dev, "%s : shutdown time = %lld seconds, cur_time = %lld, shutdown_rtc_time = %lld\n",
		__func__, shutdown_time, cur_time, data->shutdown_rtc_time);
	if (shutdown_time < SC27XX_FGU_SHUTDOWN_FOURTEEN_DAYS) {
		dev_info(data->dev, "%s : shutdown time = %lld seconds less than 14 days\n", __func__, shutdown_time);
		return false;
	}

	dev_info(data->dev, "%s : shutdown time = %lld seconds, more than 14 days\n", __func__, shutdown_time);
	return true;
}
...
static int sc27xx_fgu_get_boot_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	...
	s64  last_shutdown_time = 0;
	bool is_first_poweron = sc27xx_fgu_is_first_poweron(data);

	dev_info(data->dev, "%s: is_first_poweron = %d", __func__, is_first_poweron);
	...
	/*
	 * If system is not the first power on, we check whether the power-off
	 * time exceeds 14days. If yes, we regards it as first power-on, then
	 * re-calculate the initial battery capacity.
	 */
	if (!is_first_poweron) {
		is_first_poweron = sc27xx_fgu_boot_after_long_poweroff(data, &last_shutdown_time);
	}
	...
	if (!is_first_poweron) {
		...
		/*
		 * If system is not the first power on, we check whether the power-off
		 * time between 3days and 14 days. If yes, set the ui_cap (boot_cap) as the calibration cap.
		 * Due to the strict calibration conditions, and the cap difference needs exceeds 3%.
		 * In high probability, the calibration cap is equal to the last poweroff cap.
		 */
		if ((last_shutdown_time > SC27XX_FGU_SHUTDOWN_THREE_DAYS)
			&& (last_shutdown_time < SC27XX_FGU_SHUTDOWN_FOURTEEN_DAYS)) {
			data->boot_cap = *cap;
		}
	...
	*cap = pocv_cap;
	...
	dev_info(data->dev, "%s %d save_last_cap=%d\n", __func__, __LINE__, *cap);
...
static int sc27xx_fgu_get_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	...
	*cap = delta_cap + data->init_cap;
	...
	dev_info(data->dev, "%s,line:%d,cap=%d, data->normal_temp_cap=%d\n", __func__, __LINE__, *cap, data->normal_temp_cap);
	data->normal_temp_cap = *cap;
...
normal_cap_calc:
	sc27xx_fgu_capacity_loss_by_temperature(data, cap);
	dev_info(data->dev, "normal_cap_calc,%s,line:%d,cap=%d, data->normal_temp_cap=%d, data->uusoc_vbat=%d\n", __func__, __LINE__, *cap, data->normal_temp_cap, data->uusoc_vbat);
capacity_calibration:
	sc27xx_fgu_capacity_calibration(data, false);

	*cap -= data->uusoc_vbat;
	dev_info(data->dev, "capacity_cali, %s,line:%d,cap=%d, data->normal_temp_cap=%d, data->uusoc_vbat=%d\n",  __func__, __LINE__, *cap, data->normal_temp_cap, data->uusoc_vbat);
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		val->intval = data->total_design_mah * 1000;
		break;
				   ...
				   const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		data->total_design_mah = val->intval / 1000;
		break;
...
static int zte_fgu_get_property(struct zte_power_supply *psy,
				enum zte_power_supply_property psp,
				union power_supply_propval *pval)
{
	struct sc27xx_fgu_data *data = zte_power_supply_get_drvdata(psy);

	switch (psp) {
	case POWER_SUPPLY_PROP_BATTERY_ID:
		pr_info("%s:line%d: data->batt_id_index=%d\n", __func__, __LINE__, data->batt_id_index);
		pval->intval = data->batt_id_index;
		break;
	default:
		return -EINVAL;
	}
...
static int zte_fgu_set_property(struct zte_power_supply *psy,
			 enum zte_power_supply_property psp,
...
static int zte_fgu_property_is_writeable(struct zte_power_supply *psy,
					enum zte_power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_BATTERY_ID:
...
static void zte_fgu_external_power_changed(struct zte_power_supply *psy)
{
	vote_debug("zte usb power supply changed\n");
}

static enum zte_power_supply_property zte_fgu_psy_props[] = {
	POWER_SUPPLY_PROP_BATTERY_ID,
};

static const struct zte_power_supply_desc zte_fgu_psy_desc = {
	.name = "zte-fgu",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
	.properties = zte_fgu_psy_props,
	.num_properties = ARRAY_SIZE(zte_fgu_psy_props),
	.get_property = zte_fgu_get_property,
	.set_property = zte_fgu_set_property,
	.property_is_writeable = zte_fgu_property_is_writeable,
	.external_power_changed = zte_fgu_external_power_changed,
};
#endif
...
static void sc27xx_fgu_low_capacity_match_ocv(struct sc27xx_fgu_data *data)
{
	if (data->ocv_uv < data->min_volt_uv && data->normal_temp_cap > data->alarm_cap) {
		...
		if (data->init_cap < 0)
			data->init_cap = 0;
		dev_info(data->dev, "%s 3..\n", __func__);
	} else if (data->batt_mv < SC27XX_FGU_LOW_VBAT_REGION &&
		data->normal_temp_cap > data->alarm_cap) {
		data->uusoc_vbat += SC27XX_FGU_LOW_VBAT_UUSOC_STEP;
		dev_info(data->dev, "%s 4..\n", __func__);
	}
	dev_err(data->dev, "test..,data->ocv_uv=%d, data->min_volt_uv=%d, data->normal_temp_cap=%d"
			" data->alarm_cap=%d,data->first_calib_volt=%d, data->first_calib_cap=%d,data->batt_mv=%d"
			"data->uusoc_vbat=%d\n", data->ocv_uv, data->min_volt_uv, data->normal_temp_cap, data->alarm_cap, data->first_calib_volt, data->first_calib_cap, data->batt_mv, data->uusoc_vbat);
					      ...
					      struct sprd_battery_info *info)
{
	...
	if (info->charge_full_design_uah > 0)
		data->total_design_mah = info->charge_full_design_uah / 1000;
	else
		dev_warn(data->dev, "no fgu charge_full_design_uah support\n");

	if (info->charge_full_uah > 0)
		data->total_mah = info->charge_full_uah / 1000;
	else
	...
	data->batt_id_index = info->batt_id_index;
...
static int sc27xx_fgu_probe(struct platform_device *pdev)
{
	...
	data->batt_id_cha = devm_iio_channel_get(dev, "adc-batt-id");
	if (IS_ERR(data->batt_id_cha)) {
		dev_err(dev, "failed to get charge IIO channel, ret = %ld\n",
			PTR_ERR(data->batt_id_cha));
	}
	...
	fgu_cfg.drv_data = data;
	fgu_cfg.of_node = np;
	data->zte_battery = zte_devm_power_supply_register(dev,
						   &zte_fgu_psy_desc,
						   &fgu_cfg);
	if (IS_ERR(data->battery)) {
		dev_err(dev, "failed to register zte fgu power supply");
		ret = -ENXIO;
		goto err;
	}
#endif
	...
	mutex_unlock(&data->lock);
	pr_err("%s:line%d: OK!!! use_typec_extcon = %d\n", __func__, __LINE__, data->use_typec_extcon);
	return 0;
