...
#define SFP_DEFAULT_RATELIMIT_INTERVAL 10
#define SFP_DEFAULT_RATELIMIT_BURST 1
#define FP_PRT_DBG(FLG, fmt, arg...) {\
	if (fp_dbg_lvl & (FLG))\
		pr_info("SFP:" fmt, ##arg);\
	}
#define sfp_printk_ratelimited(fmt, ...) {\
	static DEFINE_RATELIMIT_STATE(_rs,				\
				      SFP_DEFAULT_RATELIMIT_INTERVAL,	\
				      SFP_DEFAULT_RATELIMIT_BURST);	\
									\
	if (__ratelimit(&_rs))						\
		printk(fmt, ##__VA_ARGS__);				\
	}
#define FP_PRT_RATELIMITED(FLG, fmt, arg...) {\
	if(fp_dbg_lvl&(FLG))\
		sfp_printk_ratelimited(fmt, ##arg);\
	}
#else
#define FP_PRT_DBG(FLG, fmt, arg...)
#endif
