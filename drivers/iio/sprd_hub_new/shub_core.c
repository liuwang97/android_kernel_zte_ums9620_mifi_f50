...
/* add under-panel light sensor algo start */
#include "shub_algo_cmd_notify.h"
/* add under-panel light sensor algo end */
...
struct zlog_mod_info zlog_sprd_sensorhub_dev = {
	.module_no = ZLOG_MODULE_SENSOR,
	.name = "sensor",
	.device_name = "sprd_sensorhub",
	.ic_name = "SPRD",
	.module_name = "SENSOR",
	.fops = NULL,
};
struct zlog_client *zlog_sprd_sensorhub_client = NULL;
#endif
...
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
			static int wr_zlog_times = 0;

			if (sensor->sensorhub_init_ready == true) {
				zlog_client_record(zlog_sprd_sensorhub_client, "sprd sensorhub write timeout %d\n", wr_zlog_times);
				zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_SENSORHUB_NOT_UP);
				wr_zlog_times++;
			}
#endif
...
static int shub_sipc_channel_read(struct shub_data *sensor)
{
	...
	do {
		...
		if (nread < 0) {
			...
			if (retry > 20) {
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
				static int rd_zlog_times = 0;

				if (sensor->sensorhub_init_ready == true) {
					zlog_client_record(zlog_sprd_sensorhub_client, "sprd sensorhub read timeout %d\n", rd_zlog_times);
					zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_SENSORHUB_NOT_UP);
					rd_zlog_times++;
				}
#endif
...
static int shub_sipc_channel_rdnwu(struct shub_data *sensor)
{
	...
	do {
		...
		if (nread < 0) {
			...
			if (retry > 20) {
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
				static int rdnwu_zlog_times = 0;

				if (sensor->sensorhub_init_ready == true) {
					zlog_client_record(zlog_sprd_sensorhub_client, "sprd sensorhub readnwu timeout %d\n", rdnwu_zlog_times);
					zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_SENSORHUB_NOT_UP);
					rdnwu_zlog_times++;
				}
#endif
...
static int shub_save_cali_data(struct shub_data *sensor,
					int sensor_type, u8 *data)
{
	int err;
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	int cali_data[3] = {0};

	memcpy(cali_data, data, sizeof(cali_data));
#endif

	sensor->cali_store.cmd = HAL_CALI_STORE;
	sensor->cali_store.length = CALIBRATION_DATA_LENGTH;
	sensor->cali_store.type = sensor_type;
	memcpy(sensor->cali_store.udata, data, CALIBRATION_DATA_LENGTH);

#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	switch (sensor_type) {
	case SENSOR_ACCELEROMETER:
		zlog_client_record(zlog_sprd_sensorhub_client, "acc cali data: %d %d %d\n",
			cali_data[0], cali_data[1], cali_data[2]);
	break;
	case SENSOR_GYROSCOPE:
		zlog_client_record(zlog_sprd_sensorhub_client, "gyro cali data: %d %d %d\n",
			cali_data[0], cali_data[1], cali_data[2]);
	break;
	case SENSOR_LIGHT:
		zlog_client_record(zlog_sprd_sensorhub_client, "als cali data: %d\n", cali_data[0]);
	break;
	case SENSOR_PROXIMITY:
		zlog_client_record(zlog_sprd_sensorhub_client, "prox cali data: %d\n", cali_data[0]);
	break;
	}
	zlog_client_notify(zlog_sprd_sensorhub_client, ZLOG_SENSOR_FACTORY_CALI_NO);
	dev_info(&sensor->sensor_pdev->dev, "%s: sensor_type = %d cali_data = %d %d %d send to zlog\n",
			__func__, sensor_type, cali_data[0], cali_data[1], cali_data[2]);
#endif

	dev_info(&sensor->sensor_pdev->dev, "%s: sensor_type = %d sensor->cali_store.udata = %d\n",
			__func__, sensor_type, sensor->cali_store.udata);
	err = shub_send_event_to_iio(sensor, (u8 *)&sensor->cali_store, sizeof(sensor->cali_store));
	dev_info(&sensor->sensor_pdev->dev, "%s: err = %d\n", __func__, err);
	return err;
}
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (sensor->cal_cmd == CALIB_DATA_READ) {
			...
			err = shub_save_cali_data(sensor, sensor->cal_id, buf);
			if (err < 0) {
				dev_err(&sensor->sensor_pdev->dev,
				     ...
				     const char *buf, size_t count)
{
	...
	wait_event_interruptible_timeout(sensor->sensorhub_wq,
						(sensor->sensorhub_init_ready),
						msecs_to_jiffies
						(RECEIVE_TIMEOUT_MS));
				...
				struct device_attribute *attr, char *buf)
{
	...
	/* add for sensorinfo send over */
	wait_event_interruptible_timeout(sensor->sensorinfo_wq,
						(sensor->sensor_infor_send_over),
						msecs_to_jiffies
						(RECEIVE_TIMEOUT_MS));
...
/* add opcode id for under-panel light sensor start */
static ssize_t set_brightness_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	u8 buffer[6];
	...
	ret = sscanf(buffer, "%d", &sensor->brightness);
	if (ret > 0)
		dev_info(&sensor->sensor_pdev->dev, "set_brightness = %d\n", sensor->brightness);

	shub_send_command(sensor, SENSOR_LIGHT,
			  SHUB_SET_BRIGHTNESS_SUBTYPE, buffer,
			  sizeof(buffer));

	return count;
}

static ssize_t set_brightness_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct shub_data *sensor = dev_get_drvdata(dev);
...
static DEVICE_ATTR_RW(set_brightness);

static ssize_t set_actual_fps_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	u8 buffer[6];
	...
	ret = sscanf(buffer, "%d", &sensor->actual_fps);
	if (ret > 0)
		dev_info(&sensor->sensor_pdev->dev, "set_actual_fps = %d\n", sensor->actual_fps);

	shub_send_command(sensor, SENSOR_PROXIMITY,
			  SHUB_SET_ACTUAL_FPS_SUBTYPE, buffer,
			  sizeof(buffer));

	return count;
}

static ssize_t set_actual_fps_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct shub_data *sensor = dev_get_drvdata(dev);
...
static DEVICE_ATTR_RW(set_actual_fps);
/* add opcode id for under-panel light sensor end */
...
/* add opcode id for under-panel light sensor start */
	&dev_attr_set_brightness.attr,
	&dev_attr_set_actual_fps.attr,
/* add opcode id for under-panel light sensor end */
	NULL,
};
...
/* add under-panel light sensor algo start */
static int shub_algo_cmd_notifier_fn(struct notifier_block *nb, unsigned long action, void *data)
{
	struct shub_data *sensor = container_of(nb, struct shub_data, shub_algo_cmd_notifier);
	u8 buffer[6];
	u16 value = *((u16 *)data);
	...
	switch (action) {
	case SHUB_ALGO_CMD_SET_BRIGHTNESS:
	sensor->brightness = (int)value;
	dev_info(&sensor->sensor_pdev->dev, "%s SHUB_ALGO_CMD_SET_BRIGHTNESS brightness = %d\n",
		__func__, sensor->brightness);
	shub_send_command(sensor, SENSOR_LIGHT,
			  SHUB_SET_BRIGHTNESS_SUBTYPE, buffer,
			  sizeof(buffer));
		break;
	case SHUB_ALGO_CMD_SET_ACTUAL_FPS:
	sensor->actual_fps = (int)value;
	dev_info(&sensor->sensor_pdev->dev, "%s SHUB_ALGO_CMD_SET_ACTUAL_FPS actual_fps = %d\n",
			__func__, sensor->actual_fps);
	shub_send_command(sensor, SENSOR_PROXIMITY,
			  SHUB_SET_ACTUAL_FPS_SUBTYPE, buffer,
			  sizeof(buffer));
		break;
	default:
	dev_info(&sensor->sensor_pdev->dev, "%s wrong action %d\n",
			__func__, action);
		break;
	}
...
/* add under-panel light sensor algo start */
	mcu->shub_algo_cmd_notifier.notifier_call = shub_algo_cmd_notifier_fn;
	register_shub_algo_cmd_notifier(&mcu->shub_algo_cmd_notifier);
	mcu->brightness = 128;
	mcu->actual_fps = 60;
/* add under-panel light sensor algo end */
	init_waitqueue_head(&mcu->sensorhub_wq);
	mcu->sensorhub_init_ready = false;
	init_waitqueue_head(&mcu->sensorinfo_wq);
	mcu->sensor_infor_send_over = false;

#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	zlog_sprd_sensorhub_client = zlog_register_client(&zlog_sprd_sensorhub_dev);
	if (!zlog_sprd_sensorhub_client)
		pr_err("%s zlog register client zlog_sprd_sensorhub_dev failed\n", __func__);
#endif
...
static int shub_remove(struct platform_device *pdev)
{
	...
	unregister_shub_algo_cmd_notifier(&mcu->shub_algo_cmd_notifier);  /* add under-panel light sensor algo */
	...
	kfree(mcu);
#ifdef CONFIG_VENDOR_ZTE_LOG_EXCEPTION
	if (zlog_sprd_sensorhub_client != NULL)
		zlog_unregister_client(zlog_sprd_sensorhub_client);
#endif
