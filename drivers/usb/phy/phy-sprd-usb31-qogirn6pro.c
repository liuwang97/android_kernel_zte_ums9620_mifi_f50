...
static ssize_t ssphy_ctl5_eye_pattern_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	struct sprd_ssphy *phy = dev_get_drvdata(dev);
	...
	return sprintf(buf, "0x%x\n", phy->ssphy_ctl5_eye_pattern);
}

static ssize_t ssphy_ctl5_eye_pattern_store(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	struct sprd_ssphy *phy = dev_get_drvdata(dev);
	...
	if (kstrtouint(buf, 16, &phy->ssphy_ctl5_eye_pattern) < 0)
		return -EINVAL;

	return size;
}
static DEVICE_ATTR_RW(ssphy_ctl5_eye_pattern);

static ssize_t ssphy_ctl6_eye_pattern_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	struct sprd_ssphy *phy = dev_get_drvdata(dev);
	...
	return sprintf(buf, "0x%x\n", phy->ssphy_ctl6_eye_pattern);
}

static ssize_t ssphy_ctl6_eye_pattern_store(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	struct sprd_ssphy *phy = dev_get_drvdata(dev);
	...
	if (kstrtouint(buf, 16, &phy->ssphy_ctl6_eye_pattern) < 0)
		return -EINVAL;

	return size;
}
static DEVICE_ATTR_RW(ssphy_ctl6_eye_pattern);
...
static struct attribute *usb_ssphy_attrs[] = {
	...
	&dev_attr_ssphy_ctl5_eye_pattern.attr,
	&dev_attr_ssphy_ctl6_eye_pattern.attr,
	NULL
};
...
static int sprd_ssphy_probe(struct platform_device *pdev)
{
	...
	/*
	 * The following two val is initialized for ssphy eye-pattern
	 * scope : DP/SS combo phy
	 */
	ret = of_property_read_u32(dev->of_node, "sprd,ssphy-ctl5-eye-pattern",
				   &phy->ssphy_ctl5_eye_pattern);
	if (ret < 0) {
		dev_err(dev, "unable to get ssphy-ctl5-eye-pattern node\n");
		phy->ssphy_ctl5_eye_pattern = DEFAULT_SSPHY_CTL5_EYE_PATTERN;
	}
	dev_err(dev, "ssphy_ctl5_eye_pattern = 0x%x\n",
		phy->ssphy_ctl5_eye_pattern);

	ret = of_property_read_u32(dev->of_node, "sprd,ssphy-ctl6-eye-pattern",
				   &phy->ssphy_ctl6_eye_pattern);
	if (ret < 0) {
		dev_err(dev, "unable to get ssphy-ctl6-eye-pattern node\n");
		phy->ssphy_ctl6_eye_pattern = DEFAULT_SSPHY_CTL6_EYE_PATTERN;
	}
	dev_err(dev, "ssphy_ctl6_eye_pattern = 0x%x\n",
		phy->ssphy_ctl6_eye_pattern);
