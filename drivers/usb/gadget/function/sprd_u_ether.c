...
static int sprd_get_usb_mac(void)
{
	...
	if (!sscanf(mode, "androidboot.wifimac=%s", usb_mac)) {
		return 0;
	}
	return strlen(usb_mac);
}
...
struct net_device *sprd_gether_setup_name_default(const char *netname)
{
	...
	snprintf(net->name, sizeof(net->name), "%s%%d", netname);
	boot_usbmac = sprd_get_usb_mac();
	if (boot_usbmac == usb_mac_len) {
		for (size = 0; size < ETH_ALEN  && sscanf(usb_mac + size * 2, "%2x", &value) == 1; size++) {
			dev->dev_mac[size] = value;
			dev->host_mac[size] = value;
		}
		INFO(dev, "OWNER HOST MAC %pM\n", dev->host_mac);
	} else {
