...
static void sprd_tcpm_set_typec_int_clear(void)
{
	if (g_sprd_typec_device_ops &&
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if (val->intval < port->pps_data.min_volt * 1000 ||
		    val->intval > port->pps_data.max_volt * 1000) {
			pr_err("set volt failed, max(%d), now(%d), min(%d)\n",
					port->pps_data.max_volt, (val->intval / 1000), port->pps_data.min_volt);
			ret = -EINVAL;
		} else
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (val->intval > port->pps_data.max_curr * 1000) {
			pr_err("set current failed, max(%d), now(%d)\n", port->pps_data.max_curr, (val->intval / 1000));
			ret = -EINVAL;
		} else
