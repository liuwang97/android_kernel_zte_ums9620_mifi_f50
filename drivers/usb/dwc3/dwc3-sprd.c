...
static void limit_dwc3_high_speed(struct dwc3_sprd *sdwc)
{
	struct dwc3 *dwc = platform_get_drvdata(sdwc->dwc3);

	dev_info(dwc->dev, "limit dwc3 max speed to usb20:\n");
	if (ptn38003a_mode_usb32_set(1)) {
		u32 reg;
		unsigned long flags;
		...
		reg = sdwc_readl(dwc->regs, DWC3_DSTS);
		spin_unlock_irqrestore(&dwc->lock, flags);
		dev_info(dwc->dev, "limit dwc3 max speed to usb20, DWC3_DSTS: 0x%x\n", reg);
	} else {
		dev_info(dwc->dev, "Donnot limit dwc3 max speed!\n");
	}
}
...
static int zte_calibration_mode(void)
{
	struct device_node *cmdline_node;
	const char *cmdline, *mode;
	int ret;
...
static int dwc3_sprd_otg_start_peripheral(struct dwc3_sprd *sdwc, int on)
{
	...
	if (on) {
		...
		if (zte_calibration_mode()) {
			limit_dwc3_high_speed(sdwc);
		} else {
...
static int dwc3_sprd_probe(struct platform_device *pdev)
{
	...
	if (zte_calibration_mode()) {
		dev_err(sdwc->dev, "now in calibration mode\n");
		dwc1 = platform_get_drvdata(sdwc->dwc3);
		if (!dwc1)
			return -EINVAL;
		dev_err(sdwc->dev, "boot cali mode and set hsu 2.0\n");
		dwc1->gadget.max_speed = USB_SPEED_HIGH;
	}
