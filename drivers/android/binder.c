...
#endif
#ifndef ZTE_FEATURE_CGROUP_FREEZER_GOOGLE_ASSIST
#define  ZTE_FEATURE_CGROUP_FREEZER_GOOGLE_ASSIST    false
#endif
/**** ZSW_ADD FOR CPUFREEZER end ****/
enum {
...
#if true == ZTE_FEATURE_CGROUP_FREEZER_V2
		if (!oneway) {
#if false == ZTE_FEATURE_CGROUP_FREEZER_GOOGLE_ASSIST
			struct rq_flags rf;
			struct rq *rq;

			acquire_freezer_lock();
			rq = task_rq_lock(thread->task, &rf);
			thread->task->flags |= PF_BINDER_NOFREEZE;
			thread->task->flags |= PF_NOFREEZE;
			task_rq_unlock(rq, thread->task, &rf);
			release_freezer_lock();
			freeze_state = cgroup_needunfreeze_task(thread->task);
#else
			freeze_state = cgroup_is_cgroup2_freezing(thread->task);
#endif
			if (freeze_state)
