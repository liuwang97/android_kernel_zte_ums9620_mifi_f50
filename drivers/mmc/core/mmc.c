...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 8) {
		...
		if (card->cid.manfid == 0xf4
			&& card->cid.prod_name[0] == 'A'
			&& card->cid.prod_name[1] == 'R'
			&& card->cid.prod_name[2] == 'V'
			&& card->cid.prod_name[3] == '2'
			&& card->cid.prod_name[4] == '1'
			&& card->cid.prod_name[5] == 'X') {
			printk(KERN_ERR "%s: change cmdq_depth from %d to 7.\n", __func__,
				 card->ext_csd.cmdq_depth);
				 card->ext_csd.cmdq_depth = 11;
		}
...
/*zte add for ddr size interface start*/
...
int zte_ddrsize = 0;
static int __init zte_ddrsize_get(char *str)
{
	zte_ddrsize = atoi(str);
	pr_debug("ddr size from cmdline: %d\n", zte_ddrsize);
	return 0;
}
__setup("androidboot.ddrsize=", zte_ddrsize_get);

typedef struct _ddr_manf_info1 {
	int id;
	char *name;
} ddr_manf_info1;

static ddr_manf_info1 ddr_list[] = {
	{0x01, "Samsung"},
	{0x02, "Infineon"},
	{0x03, "Elpida"},
	{0x04, "Etpon"},
...
static int ddr_id_proc_show(struct seq_file *m, void *v)
{
	...
	ptr = strstr(saved_command_line, "zte_mr5=");
	if (ptr) {
	ptr += strlen("zte_mr5=");
	ddr_id = simple_strtol(ptr, NULL, 16);

	for (i = 0; i < ARRAY_SIZE(ddr_list); i++) {
		if (ddr_list[i].id == ddr_id) {
			manfname = ddr_list[i].name;
			break;
		}
	}
	}

	/*get ddrsize from cdmline first*/
	if (zte_ddrsize != 0) {
		seq_printf(m,
			"%s-NA-NA-%dGB-LPDDR4\n",
			manfname, (zte_ddrsize/1024));
	} else {
	memory = totalram_pages() << (PAGE_SHIFT - 10);
	/*1G=1048576K*/
	memory = memory/(1024*1024);/*1G=1048576K*/
	memory += 1;
	seq_printf(m,
		"%.8s-NA-NA-%dGB-%.8s\n",
...
int mmc_attach_mmc(struct mmc_host *host)
{
	...
	if (err)
		goto err;
/*[ECID:000000] ZTEBSP wangxiaomei record eMMC info into /proc/driver/emmc file, 20131108, begin*/
	#ifdef ZTE_SUPPORT_UFS
	if (true == ZTE_SUPPORT_UFS)
	    emmc_proc_init = true;
	#endif
